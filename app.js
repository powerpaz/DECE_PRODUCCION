...
function mapRowsToData(rows, idx) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    data.push({ lat, lng, code: typeCode, codGDECE, name, dist, zona, students, amie, provincia, codProvincia, canton, codCanton });
...
function drawNucleos(nucleos, selected) {
  nucleos.forEach((n, ni) => {
    ...
    const marker = L.circleMarker([n.lat, n.lng], { radius: isSelected ? 10 : 6, fillColor: isSelected ? '#58a6ff' : '#a371f7', color: '#fff', weight: 2, opacity: 1, fillOpacity: isSelected ? 0.9 : 0.7, renderer: canvasRenderer });
...
function drawSatellites(satellites, satCandidates, selected) {
  ...
  satellites.forEach((s, si) => {
    ...
    const marker = L.circleMarker([s.lat, s.lng], {
      radius: 4,
      fillColor: isCovered ? '#f0883e' : '#f85149',
      color: '#fff',
      weight: 1,
      opacity: 1,
      fillOpacity: 0.8,
...
function createSatellitePopup(s, distMetersOrNull) {
  ...
  return `<div class="popup-title">📍 Satélite</div><div class="popup-content"><div class="popup-row"><span class="popup-label">Institución:</span><span class="popup-value">${escapeHTML(s.name)}</span></div><div class="popup-row"><span class="popup-label">Distrito:</span><span class="popup-value">${escapeHTML(s.dist)}</span></div><div class="popup-row"><span class="popup-label">Estado:</span><span class="popup-value" style="color:${covered ? "#f0883e" : "#f85149"}">${covered ? "✓ Cubierto" : "✗ Sin cobertura"}</span></div>${covered ? `<div class="popup-row"><span class="popup-label">Distancia:</span><span class="popup-value">${(distMetersOrNull/1000).toFixed(2)} km</span></div>` : ''}<div class="popup-row"><span class="popup-label">Estudiantes:</span><span class="popup-value" style="color:#d29922">${(s.students || 0).toLocaleString()}</span></div></div>`;
}
