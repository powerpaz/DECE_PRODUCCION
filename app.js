...
// Requisito: si supera 7 km, NO hay atención (ni normal, ni forzada)
const BUFFER_RADIUS_M = 7000; // 7 km
const ORPHAN_WARNING_DISTANCE_M = 7000; // 7 km: umbral para marcar fuera de alcance
const ORPHAN_BUFFER_NUCLEO_MAX_M = 7000; // 7 km: máximo satélite→núcleo (solo fiscales/seleccionados) para cubrir buffers B#
const ORPHAN_MAX_DISTANCE_M = 7000; // corte duro: no conectar huérfanos si supera 7 km
...
const bufferRenderer = L.canvas({ padding: 0.05 });       // buffers (arrastre fluido sin afectar marcadores)
const connectionRenderer = L.canvas({ padding: 0.5 });    // conexiones
const buffersRenderer = L.svg({ padding: 0.1 });          // buffers
...
function createCustomBuffer(lat, lng) {
  customBufferCounter++;
  const circle = L.circle([lat, lng], { radius: BUFFER_RADIUS_M, color: '#a371f7', fillColor: '#a371f7', weight: 2, opacity: 0.7, fillOpacity: 0.15, renderer: bufferRenderer });
...
// ==================== BUFFER HANDLES (DRAG 100% FIABLE) ====================
// Nota: Los círculos (L.circle) en renderer canvas pueden fallar en hit-detection bajo carga.
// Para garantizar arrastre, usamos un marcador draggable invisible/pequeño en el centro del buffer.
...
function enableBufferEditing() {
  ...
  // Mantener el mapa pan-eable normalmente. El arrastre de buffers se hace desde el handle.
  try { map.dragging.enable(); } catch (e) {}
  try { map.getContainer().style.cursor = 'default'; } catch (e) {}
  showNotification("Modo edición: arrastra el punto (handle) en el centro del buffer.", "info");
  // Garantizar handles para arrastre (más confiable que eventos sobre círculos en canvas)
  editableBuffers.forEach((data, ni) => ensureBufferHandle(data, false, ni));
  customBuffers.forEach((buffer) => ensureBufferHandle(buffer, true));
  setHandlesEditingEnabled(true);
...
function regenerateAnimations() {
  ...
  // ✅ SOLO núcleos fiscales seleccionados otorgan cobertura y generan conexiones.
  const nucleoCenters = getActiveNucleoCentersOnly();
  const nucleoGrid = buildPointGrid(nucleoCenters);

  satellites.forEach((sat, si) => {
    let assigned = null;

    // Si está dentro de un buffer huérfano B#, su conexión real depende del núcleo fiscal asignado (≤ 7 km)
    const orphanBid = orphanBufferCoverageState?.satInsideOrphan?.get?.(si);
    if (orphanBid) {
      const a = orphanBufferCoverageState?.satAssignments?.get?.(si);
      if (a && Number.isFinite(a.ni)) {
        const nuc = nucleos?.[a.ni];
        if (nuc) {
          const pos = getCurrentNucleoLatLng(a.ni, nuc);
          assigned = { ni: a.ni, lat: pos.lat, lng: pos.lng, distance: a.distanceMeters };
        }
      }
    }

    // Cobertura normal: núcleo fiscal seleccionado más cercano dentro de 7 km
    if (!assigned) {
      const hit = findClosestInGridWithin(nucleoGrid, sat.lat, sat.lng, BUFFER_RADIUS_M, 1);
      if (hit && hit.point && Number.isFinite(hit.point.ni)) {
        assigned = { ni: hit.point.ni, lat: hit.point.lat, lng: hit.point.lng, distance: hit.distance };
      }
    }

    if (assigned) {
      const line = L.polyline(
        [[assigned.lat, assigned.lng], [sat.lat, sat.lng]],
        {
          color: '#111827',
          weight: 3,
          opacity: 0.85,
  ...
  // Incluir conexiones forzadas (ATENCIÓN DISTRITAL, solo ≤7km ya se filtra en analyzeOrphans)
  drawForcedConnections(newLines, tempGroup);
...
function analyzeOrphans() {
  ...
  satellites.forEach((sat, si) => {
    ...
    if (closest) {
      // ✅ Corte duro: si supera 7 km, NO se asigna atención.
  ...
  orphanAnalysis.stats = {
    ...
    tooFar: orphanAnalysis.unservedSatellites.size,
  ...
  console.log("Fuera de alcance (>7km):", orphanAnalysis.unservedSatellites.size);
...
function updateOrphanPanel() {
  ...
  panel.innerHTML = `
    ...
    <div class="orphan-stat warn">
      <div class="stat-label">Fuera de alcance (&gt;7km)</div>
...
const satellitesOther = satellitesAll.filter(s => !isExcludedGroup1to50(s) && !isFiscalInstitution(s));
const satellites = satellitesAll.filter(s => isFiscalInstitution(s) && !isExcludedGroup1to50(s)); // análisis

const nucleosOther = nucleos.filter(n => !isFiscalInstitution(n));
nucleos = nucleos.filter(n => isFiscalInstitution(n)); // análisis

console.log(`[FILTER] Satélites análisis (Fiscales, >50): ${satellites.length}/${satellitesAll.length}`);
...
function drawNucleos(nucleos, selected) {
  nucleos.forEach((n, ni) => {
    ...
    // Colores por sostenimiento (Fiscal=rojo, etc). Diferenciamos selección con tamaño/borde.
...
function drawBuffersEditable(nucleos, selected, nucleoStats) {
  ...
  selected.forEach(ni => {
    ...
    const circle = L.circle([lat, lng], { radius: BUFFER_RADIUS_M, color: '#9333ea', fillColor: '#9333ea', weight: 2, opacity: 0.6, fillOpacity: 0.08, renderer: bufferRenderer });
    circle.addTo(layers.buffers);
...
function restoreCustomBuffer(saved) {
  customBufferCounter++;
  const circle = L.circle([saved.lat, saved.lng], { radius: BUFFER_RADIUS_M, color: '#a371f7', fillColor: '#a371f7', weight: 2, opacity: 0.7, fillOpacity: 0.15, renderer: bufferRenderer });
  circle.addTo(layers.buffers);
...
function drawSatellites(satellites, satCandidates, selected) {
  ...
  satellites.forEach((s, si) => {
    ...
    // 🔎 Criterio visual (más evidente):
    // - CON cobertura: rojo sólido + borde blanco
    // - SIN cobertura: rojo translúcido + borde negro + aro más grande
    const marker = L.circleMarker([s.lat, s.lng], {
      radius: normalCovered ? 7 : 8,
      fillColor: style.fill,
      color: normalCovered ? '#ffffff' : '#000000',
      weight: normalCovered ? 2 : 3,
      dashArray: normalCovered ? null : '5,6',
      opacity: 1,
      fillOpacity: normalCovered ? 0.95 : 0.28,
      renderer: canvasRenderer
    });

    marker.bindPopup(createSatellitePopup(s, { normalCovered, normalDist, forced, tooFar }));
...
function computeStatistics(nucleos, satellites, satCandidates, selected, nucleoStats) {
  ...
  // - Cobertura normal = dentro de algún buffer (≤ 7 km) con posición ACTUAL
  // - "Sin cobertura" = satélites rojos (fuera de 7 km)
  const activeCenters = getActiveNucleoCentersOnly();
  const activeGrid = buildPointGrid(activeCenters);
...
function setupControls() {
  ...
  const layerNames = [
    ...
    "Núcleos Fiscales",
    "Núcleos No Fiscales",
    "Satélites Fiscales (con cobertura)",
    "Satélites Fiscales (sin cobertura)",
