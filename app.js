...
function exportToExcel() {
  try {
    ...
    showNotification("✅ Exportación lista (Registros_GIEE + Vista_Mapa + Resumen).", "success");
  } catch (e) {
...
// ===============================
// BUSCADOR DE AMIE (CENTRAR Y ABRIR POPUP)
// ===============================
function findFeatureByAMIE(amieCode) {
    let found = null;
    map.eachLayer(function (layer) {
        if (layer.feature && layer.feature.properties) {
            const p = layer.feature.properties;
            const code = p.AMIE || p.CODAMIE || p.CODIGO_INS;
            if (code && String(code).toUpperCase() === amieCode) {
                found = layer;
            }
        }
    });
...
document.addEventListener("DOMContentLoaded", () => {
    const input = document.getElementById("amieSearchInput");
    const btn = document.getElementById("amieSearchBtn");
    const msg = document.getElementById("amieSearchMsg");
    ...
    btn.addEventListener("click", () => {
        ...
        if (!amie) {
            msg.textContent = "Ingrese un código AMIE válido.";
            return;
        }

        const layer = findFeatureByAMIE(amie);
        if (!layer) {
            msg.textContent = "AMIE no encontrado en el mapa.";
            return;
        }

        msg.textContent = "AMIE encontrado. Centrando mapa…";
...
// ===============================
// BUSCADOR AMIE - INSERCIÓN SEGURA EN KPI
// ===============================
function insertAMIESearchAfterCoverage() {
    const kpiTitles = document.querySelectorAll('.kpi-title, .kpi-header, h3, h4');
    let coverageBlock = null;

    kpiTitles.forEach(el => {
        if (el.textContent && el.textContent.toUpperCase().includes('COBERTURA DE SATÉLITES')) {
            coverageBlock = el.closest('.kpi-card') || el.parentElement;
        }
    });

    if (!coverageBlock) return;

    if (document.getElementById('amie-search-card')) return;

    const card = document.createElement('div');
    card.className = 'kpi-card';
    card.id = 'amie-search-card';
    card.style.marginTop = '12px';

    card.innerHTML = `
        <div class="kpi-title">🔎 Buscar Institución (AMIE)</div>
        <input id="amieSearchInput" type="text"
            placeholder="Coloque Cod AMIE ASI: 01H01581"
            style="width:100%;padding:8px;border-radius:6px;border:1px solid #334155;background:#0b1220;color:#e5e7eb;">
        <button id="amieSearchBtn"
            style="margin-top:8px;width:100%;padding:8px;border-radius:6px;border:none;background:#22c55e;color:#022c22;font-weight:bold;cursor:pointer;">
            Buscar y centrar
        </button>
        <div id="amieSearchMsg" style="margin-top:6px;font-size:12px;color:#94a3b8;"></div>
    `;

    coverageBlock.after(card);

    document.getElementById('amieSearchBtn').onclick = () => {
        const amie = document.getElementById('amieSearchInput').value.trim().toUpperCase();
        const msg = document.getElementById('amieSearchMsg');
        ...
        map.eachLayer(layer => {
            if (layer.feature && layer.feature.properties) {
                const p = layer.feature.properties;
                const code = p.AMIE || p.CODAMIE || p.CODIGO_INS;
                if (code && String(code).toUpperCase() === amie) {
                    found = layer;
                }
            }
        });
        ...
        msg.textContent = 'AMIE encontrado. Centrando mapa…';
        if (found.getBounds) map.fitBounds(found.getBounds(), { maxZoom: 16 });
        else if (found.getLatLng) map.setView(found.getLatLng(), 16);
        if (found.openPopup) found.openPopup();
    };
}

document.addEventListener('DOMContentLoaded', () => {
    setTimeout(insertAMIESearchAfterCoverage, 500);
    setTimeout(insertAMIESearchAfterCoverage, 1500);
});
