...
function performSpatialJoin() {
  ...
  // Mezclamos instituciones para el spatial join
  ...
  // ===== Buffers de núcleos seleccionados (editables) =====
  editableBuffers.forEach((data, ni) => {
    ...
    const nuc = data.nucleo || nucleos?.[ni] || null;

    buffers.push({
      bufferId: `buffer_nucleo_${ni}`,
      bufferName: nuc?.name || `Núcleo ${ni}`,
      bufferOriginalName: nuc?.name || `Núcleo ${ni}`,
      bufferAlias: '',
      isOrphanBuffer: false,

      // Metadata del núcleo asociado (aunque el buffer se mueva)
      nucleoIndex: ni,
      nucleoAmie: nuc?.amie || '',
      nucleoName: nuc?.name || '',
      ...
      originalLat: nuc?.lat ?? null,
      originalLng: nuc?.lng ?? null,
      wasMoved: (Number.isFinite(nuc?.lat) && Number.isFinite(nuc?.lng)) ? (bufferPos.lat !== nuc.lat || bufferPos.lng !== nuc.lng) : false,
  ...
  customBuffers.forEach(buffer => {
    ...
    buffers.push({
      ...
      centerLng: bufferPos.lng,
      radiusMeters: BUFFER_RADIUS_M,
      originalLat: buffer.originalPos?.lat ?? null,
      originalLng: buffer.originalPos?.lng ?? null,
      wasMoved: (buffer.originalPos && Number.isFinite(buffer.originalPos.lat) && Number.isFinite(buffer.originalPos.lng))
        ? (bufferPos.lat !== buffer.originalPos.lat || bufferPos.lng !== buffer.originalPos.lng)
        : false,
  ...
  // ============================
  // Identificar BUFFERS HUÉRFANOS (satélites SIN ningún núcleo dentro del radio)
  // y asignar alias B1, B2, B3... (orden estable: Norte→Sur, Oeste→Este)
  // ============================
  const orphanBuffersSorted = buffers
    .filter(b => (b.nucleosCount === 0) && (b.satellitesCount > 0))
    .sort((a, b) => (b.centerLat - a.centerLat) || (a.centerLng - b.centerLng) || String(a.bufferId).localeCompare(String(b.bufferId)));

  orphanBuffersSorted.forEach((b, idx) => {
    const alias = `B${idx + 1}`;
    b.bufferAlias = alias;
    b.isOrphanBuffer = true;
    b.bufferName = alias; // lo que se mostrará en export y (si aplica) en satélites como buffer más cercano
  });

  // Mapa rápido: bufferId -> etiqueta final (alias o nombre)
  const bufferLabelById = new Map(buffers.map(b => [b.bufferId, b.bufferName]));

  const allSatellitesInBuffers = buffers.reduce((sum, b) => sum + (b.satellitesCount || 0), 0);
  ...
  const centerInfos = activeCenters.map(c => {
    const bufferId = (c.kind === 'nucleo') ? `buffer_nucleo_${c.ni}` : (c.id || '');
    const labelFromBuffer = bufferLabelById.get(bufferId) || '';
    ...
    if (c.kind === 'nucleo') {
      ...
      return {
        ...c,
        bufferId,
        label: labelFromBuffer || nuc?.name || `Núcleo ${c.ni}`,
        amie: nuc?.amie || '',
        kindName: 'Núcleo'
      };
    }
    ...
    if (c.kind === 'custom') {
      ...
      return {
        ...c,
        bufferId,
        label: labelFromBuffer || b?.name || c.id || 'Buffer personalizado',
        amie: '',
        kindName: 'Personalizado'
      };
    }

    return { ...c, bufferId, label: labelFromBuffer || 'Buffer', amie: '', kindName: String(c.kind || 'Buffer') };
  });
  ...
  const satellitesAllExport = (satellites || []).map((sat, si) => {
    ...
    return {
      ...
      nearestBuffer: nearest.center ? {
        bufferId: nearest.center.bufferId || '',
  ...
  const orphanBuffersList = orphanBuffersSorted.map(b => ({
    bufferId: b.bufferId,
    alias: b.bufferAlias,
    originalName: b.bufferOriginalName,
    isCustom: b.isCustom,
    centerLat: b.centerLat,
    centerLng: b.centerLng,
    satellitesCount: b.satellitesCount,
    totalStudents: b.totalStudents
  }));

  return {
    exportDate: new Date().toLocaleString('es-EC'),
    summary: {
      totalBuffers: buffers.length,
      originalBuffers: editableBuffers.size,
      customBuffers: customBuffers.length,
      orphanBuffers: orphanBuffersSorted.length,
      ...
      totalSatellites: allSatellitesInBuffers,
      totalStudents: totalStudentsCovered,
      coveragePercent: satellites.length > 0 ? ((allSatellitesInBuffers / satellites.length) * 100).toFixed(1) : 0
    },

    buffers,

    orphanBuffers: orphanBuffersList,
...
function exportToExcel() {
  ...
  const summaryData = [['REPORTE DE ANÁLISIS ESPACIAL DECE'],['Fecha:', data.exportDate],[''],['MÉTRICAS'],
    ...
    ['Buffers huérfanos (sin núcleo):', data.summary.orphanBuffers],['Total AMIEs:', data.summary.totalAMIEs],
  ...
  const buffersHeaders = ['ID Buffer','Alias','Nombre','Nombre Original','Tipo','¿Huérfano?','AMIE Núcleo','Nombre Núcleo','Sostenimiento Núcleo','Lat Centro','Lng Centro','Radio (m)','Fue Movido','Total AMIEs','Núcleos','Satélites','Estudiantes'];
  const buffersData = data.buffers.map(b => [
    b.bufferId,
    b.bufferAlias || '',
    b.bufferName,
    b.bufferOriginalName || '',
    b.isCustom ? 'Personalizado' : 'Original',
    b.isOrphanBuffer ? 'Sí' : 'No',
    b.nucleoAmie || '',
    b.nucleoName || '',
  ...
  const instHeaders = ['Buffer','Buffer Alias','Buffer Nombre Original','AMIE','Nombre','Sostenimiento','Tipo','COD_GDECE','Lat','Lng','Distancia(m)','Distancia(km)','Estudiantes','Distrito'];
  const instData = [];
  data.buffers.forEach(buffer => buffer.institutions.forEach(inst => instData.push([
    buffer.bufferName,
    buffer.bufferAlias || '',
    buffer.bufferOriginalName || '',
    inst.amie,
    inst.name,
    inst.sostenimiento || '',
    inst.typeName,
  ...
  // --- Buffers huérfanos (sin núcleo) ---
  if (Array.isArray(data.orphanBuffers) && data.orphanBuffers.length) {
    const obHeaders = ['Alias','Nombre Original','Tipo','Lat Centro','Lng Centro','Satélites','Estudiantes'];
    const obRows = data.orphanBuffers.map(b => [
      b.alias || '',
      b.originalName || '',
      b.isCustom ? 'Personalizado' : 'Original',
      b.centerLat,
    ...
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([obHeaders, ...obRows]), 'Buffers_Huérfanos');
  }
...
function exportToCSV() {
  ...
  // ===== CSV 1: Detalle por institución (buffer x institución) =====
  const headers = [
    'Buffer_ID','Buffer_Alias','Buffer_Nombre','Buffer_Nombre_Original','Buffer_Tipo','Buffer_Lat','Buffer_Lng',
    'AMIE','Institucion_Nombre','Sostenimiento','Institucion_Tipo','COD_GDECE','Inst_Lat','Inst_Lng',
    'Distancia_m','Distancia_km','Estudiantes','Distrito'
  ];
  ...
  data.buffers.forEach(buffer => {
    buffer.institutions.forEach(inst => {
      rows.push([
        buffer.bufferId,
        `"${String(buffer.bufferAlias || '').replace(/"/g,'""')}"`,
        `"${String(buffer.bufferName || '').replace(/"/g,'""')}"`,
        `"${String(buffer.bufferOriginalName || '').replace(/"/g,'""')}"`,
  ...
  // ===== CSV 2: Resumen de satélites (uno por registro) =====
  if (Array.isArray(data.satellitesAll) && data.satellitesAll.length) {
    const satHeaders = [
      'AMIE','Nombre','Sostenimiento','Estudiantes','Distrito','Lat','Lng',
      'Cubierto','BufferMasCercano','TipoBuffer','DistBufferKm',
      'ConexionForzada','NucleoAsignado','AMIE_NucleoAsignado','DistForzadaKm','Extendida>7km'
    ];
