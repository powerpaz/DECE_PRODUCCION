...
function exportToExcel() {
  ...
  const instHeaders = ['Buffer','Alias Huérfano','Núcleo AMIE','Núcleo Nombre','Núcleo Sostenimiento','AMIE','Nombre','Sostenimiento','Tipo','COD_GDECE','Lat','Lng','Distancia(m)','Distancia(km)','Tiempo(min aprox)','Estudiantes','Distrito'];
  const instData = [];
  ...
  // --- Registros administrativos (formato como tu tabla GIEE) ---
  // Fuente: dataset completo parseado desde el CSV (todas las filas válidas).
  const rawRows = window._rawRowsForExport || [];
  if (Array.isArray(rawRows) && rawRows.length) {
    const gHeaders = ['IE_Fiscales','Jornadas','Grupo_DECE','COD_GDECE','COORD_DECE','PO_ProfDECE','DECE NUCLEO','AMIE_NUCLEO','PO_ProfDECE+G2'];
    const toDash = (v) => {
    ...
    const gRows = rawRows.map(r => {
      const ie = (r.ieFiscalesRaw ?? (r.ieFiscales ? '1' : '')).toString().trim();
      const j = (r.jornadasRaw ?? (r.jornadas ?? '')).toString().trim();
      const grupo = (r.grupoDece ?? '').toString().trim();
      const cod = (r.codGDECE ?? '').toString().trim();
      const coord = (r.coordDeceRaw ?? (r.coordDece ?? '')).toString().trim();
      const po = (r.profDeceRaw ?? (r.profDece ?? '')).toString().trim();

      // ✅ Completar desde lógica del mapa (si aplica)
      const amie = (r.amie ?? '').toString().trim();
      const dnFromMap = amie ? gieeAssign.deceNucleoByAmie.get(amie) : null;
      const amieNFromMap = amie ? gieeAssign.amieNucleoByAmie.get(amie) : null;

      // dn: indicador numérico (1/0) para el cálculo PO_ProfDECE+G2
      const dn = (dnFromMap != null ? String(dnFromMap) : (r.deceNucleoRaw ?? (r.deceNucleo ?? '')).toString()).trim();

      // amieN: AMIE del núcleo asignado / más cercano (según estado actual del mapa)
      const amieN = (amieNFromMap != null ? String(amieNFromMap) : (r.amieNucleo ?? '')).toString().trim();

      const sum = toNum0(po) + toNum0(dn);
      return [
        toDash(ie),
        toDash(j),
        (grupo || '-'),
        ...
        toDash(coord),
        toDash(po),
        // DECE NUCLEO: el usuario requiere ver aquí el AMIE del núcleo (no el 1/0)
        toDash(amieN),
        // AMIE_NUCLEO: se mantiene también con el AMIE del núcleo para consistencia
        toDash(amieN),
        (sum ? sum : '-')
      ];
    });

    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([gHeaders, ...gRows]), 'Registros_GIEE');
  }

  // --- Buffers huérfanos (B1, B2, ...) ---
  if (Array.isArray(data.orphanBuffers) && data.orphanBuffers.length) {
    const oHeaders = ['Alias','Buffer_ID','Lat Centro','Lng Centro','Satélites','Estudiantes','Núcleo fiscal (AMIE)','Núcleo fiscal (Nombre)','Sostenimiento núcleo','Satélites cubiertos ≤7km','Satélites en buffer','Dist. prom (km)','Dist. máx (km)','Tiempo prom (min)','Tiempo máx (min)'];
    const oRows = data.orphanBuffers.map(b => [b.alias, b.bufferId, b.centerLat, b.centerLng, b.satellitesCount, b.totalStudents, b.nucleoAmie || '', b.nucleoNombre || '', b.nucleoSostenimiento || '', b.coberturaSatellites7km ?? '', b.coberturaSatellitesTotal ?? '', b.coberturaAvgKm ?? '', b.coberturaMaxKm ?? '', b.coberturaAvgMin ?? '', b.coberturaMaxMin ?? '']);
...
function analyzeOrphans() {
  ...
  let normalCovered = 0;
  // ✅ Ya no existe “asignación forzada”. Si está >7.5 km simplemente NO recibe atención.
  let forcedCovered = 0;
  ...
  satellites.forEach((sat, si) => {
    ...
    // 2) Fuera de cobertura: encontrar el núcleo fiscal más cercano (para referencia),
    //    pero NO asignar atención si supera el corte de 7.5 km.
    const closest = findClosestNucleoForOrphan(sat, lookup, true);

    // Siempre queda sin atención (está fuera de buffers). Guardamos el más cercano solo para reporte/KPI.
  ...
  // Stats
  // En este modelo:
  // - Cubierto = tiene algún núcleo fiscal seleccionado dentro de 7.5 km
  // - Fuera de alcance = no tiene cobertura (sin atención)
  orphanAnalysis.stats = {
    eligibleTotal,
    total: eligibleTotal,
    normalCovered: normalCovered,
    forcedCovered: 0,
    tooFar: orphanAnalysis.orphanSatellites.size,
    unserved: orphanAnalysis.orphanSatellites.size,
    normalPercent: eligibleTotal ? ((normalCovered / eligibleTotal) * 100).toFixed(2) : "0.00",
    totalPercent: eligibleTotal ? ((normalCovered / eligibleTotal) * 100).toFixed(2) : "0.00"
  };
  ...
  console.log("Forzados:", 0);
  console.log("TOTAL:", normalCovered, `(${orphanAnalysis.stats.totalPercent}%)`);
  console.log("Fuera de alcance (>7.5km):", orphanAnalysis.orphanSatellites.size);
...
function resolveColumnIndexes(headerRow) {
  ...
  const idxJornadas = find(["jornadas"]);
...
function drawNucleos(nucleos, selected) {
  nucleos.forEach((n, ni) => {
    ...
    const nucleoStyle = { fill: '#111827', stroke: '#e5e7eb' };
...
function drawSatellites(satellites, satCandidates, selected) {
  ...
  satellites.forEach((s, si) => {
    ...
    // - CON cobertura: verde
    // - SIN cobertura: rojo
    const coveredFill = '#22c55e';
    const uncoveredFill = '#f85149';
    const marker = L.circleMarker([s.lat, s.lng], {
      radius: normalCovered ? 7 : 8,
      fillColor: normalCovered ? coveredFill : uncoveredFill,
      color: normalCovered ? '#ffffff' : '#000000',
      weight: normalCovered ? 2 : 3,
      dashArray: normalCovered ? null : '5,6',
      opacity: 1,
      fillOpacity: normalCovered ? 0.95 : 0.85,
      renderer: canvasRenderer
    });
