...
function buildCoverageZonesRowsForExport() {
  try {
    ...
    for (const item of groupArr) {
      const g = item.g;
      const nucAmies = g.map(x => x.amie).filter(Boolean);
      ...
      rows.push({
        ZONA_ID: `Z${k}`,
        AMIE_NUCLEOS_DENTRO_DEL_BUFFER: nucAmies.join(', '),
        TOTAL_NUCLEOS: g.length,
        SATELITES_FISCALES: satAmies.join(', '),
        TOTAL_SATELITES: satAmies.length,
        CENTRO_LAT: Number(item.clat.toFixed(6)),
        CENTRO_LNG: Number(item.clng.toFixed(6))
      });
      k++;
    }
...
function buildBuffersBRowsForExport() {
  try {
    ...
    const sorted = list.slice().sort((a, b) => (aliasNum(a.alias) - aliasNum(b.alias)) || String(a.alias).localeCompare(String(b.alias)));

    for (const ob of sorted) {
      const amies = bufToSat.get(ob.bufferId) || [];
      const uniq = Array.from(new Set(amies)).sort();
      rows.push({
        BUFFER_ALIAS: String(ob.alias || ob.bufferId),
        SATELITES_FISCALES: uniq.join(', '),
        TOTAL_SATELITES: uniq.length,
        CENTRO_LAT: Number((ob.centerLat ?? '').toString() ? Number(ob.centerLat).toFixed(6) : ''),
        CENTRO_LNG: Number((ob.centerLng ?? '').toString() ? Number(ob.centerLng).toFixed(6) : '')
      });
    }
...
function exportToExcel() {
  try {
    ...
    const analysis = window._exportData || performSpatialJoin();
    ...
    XLSX.utils.book_append_sheet(wb, ws, 'Registros_GIEE');

    const ws2 = XLSX.utils.json_to_sheet(explainRows);
    XLSX.utils.book_append_sheet(wb, ws2, 'Vista_Mapa');

    const ws3 = XLSX.utils.aoa_to_sheet(resumen);
    XLSX.utils.book_append_sheet(wb, ws3, 'Resumen');

    // 4) Hoja Cobertura_Zonas (agrupa núcleos por solape de buffers y lista satélites dentro)
    const zonasRows = buildCoverageZonesRowsForExport();
    const ws4 = XLSX.utils.json_to_sheet(zonasRows);
    XLSX.utils.book_append_sheet(wb, ws4, 'Cobertura_Zonas');

    // 5) Hoja Buffers_B (satélites fiscales contenidos en Buffers B#)
    const bRows = buildBuffersBRowsForExport();
    const ws5 = XLSX.utils.json_to_sheet(bRows);
    XLSX.utils.book_append_sheet(wb, ws5, 'Buffers_B');
    ...
    showNotification("✅ Exportación lista (Registros_GIEE + Vista_Mapa + Resumen + Cobertura_Zonas + Buffers_B).", "success");
  } catch (e) {
...
function setupControls() {
  ...
  // estado inicial
  try { setKpiHidden(localStorage.getItem('kpiHidden') === '1'); } catch(_) {}
...
/* =========================================================
   ESPEJO TOTAL (MAPA → KPIs → VISTA PREVIA → EXPORTACIÓN)
   - No recalcula geometría
   - No rehace joins espaciales
   - Respeta posiciones (usa estado final de Leaflet)
   ========================================================= */


function __safeArr(v) {
  return (typeof v !== "undefined" && Array.isArray(v)) ? v : [];
}
function __safeMap(v) {
  return (v && typeof v.forEach === "function") ? v : null;
}
function __safeList(v) {
  return (v && Array.isArray(v)) ? v : [];
}
/* ---------- 1) ESTADO FINAL DEL MAPA (BASE) ---------- */
function buildCoberturaZonasFromMapState() {
  const rows = [];
  const __eb = (typeof editableBuffers !== 'undefined') ? editableBuffers : [];
  const __cb = (typeof customBuffers !== 'undefined') ? customBuffers : [];
  const allBuffers = [...__safeArr(__eb), ...__safeArr(__cb)];

  allBuffers.forEach((buffer, idx) => {
    const bufferId = buffer.id || `B${idx + 1}`;
    const center = buffer.circle.getLatLng();
    const wasMoved = buffer.wasMoved === true;
    const nucleo = buffer.nucleo || null;

    const satsCovered = [];
    const satsUncovered = [];
    const satsOrphan = [];

    // Satélites según estado REAL ya resuelto en el mapa (asignaciones finales)
    __safeMap((typeof orphanBufferCoverageState !== 'undefined' && orphanBufferCoverageState) ? orphanBufferCoverageState.satAssignments : null)?.forEach((info, satId) => {
      if (info.bufferId === bufferId) {
        info.covered ? satsCovered.push(satId) : satsUncovered.push(satId);
      }
    });

    // Satélites en buffers huérfanos (B#)
    __safeList((typeof satInsideOrphan !== 'undefined') ? satInsideOrphan : []).forEach(sat => {
      if (sat.bufferId === bufferId) satsOrphan.push(sat.AMIE);
    });

    const tipoBuffer = nucleo
      ? (satsCovered.length > 0 ? "BUFFER_NUCLEO_SATELITE" : "BUFFER_NUCLEO")
      : "BUFFER_HUERFANO";

    const aliasMapa = nucleo
      ? (tipoBuffer === "BUFFER_NUCLEO_SATELITE" ? "BSN" : bufferId)
      : bufferId;

    rows.push({
      ZONA_ID: bufferId,
      ALIAS_MAPA: aliasMapa,
      TIPO_BUFFER: tipoBuffer,
      AMIE_NUCLEO: nucleo ? nucleo.AMIE : "SIN_NUCLEO",
      TOTAL_NUCLEOS: nucleo ? 1 : 0,
      TOTAL_SATELITES: satsCovered.length + satsUncovered.length + satsOrphan.length,
      SATELITES_CUBIERTOS: satsCovered.length,
      SATELITES_NO_CUBIERTOS: satsUncovered.length,
      SATELITES_HUERFANOS: satsOrphan.length,
      BUFFER_MOVIDO: wasMoved ? "SI" : "NO",
...
/* ---------- 2) KPIs (ESPEJO DEL MAPA) ---------- */
function calcularKPIsDesdeMapa() {
  const zonas = buildCoberturaZonasFromMapState();
  const TOTAL_SATELITES_BASE = 1415; // Fiscal 51–120

  const satCubiertos = zonas.reduce((s, z) => s + (Number(z.SATELITES_CUBIERTOS) || 0), 0);
  const satNoCubiertos = Math.max(0, TOTAL_SATELITES_BASE - satCubiertos);

  const coberturaPct = TOTAL_SATELITES_BASE > 0
    ? (satCubiertos / TOTAL_SATELITES_BASE) * 100
    : 0;

  const nucleosSet = new Set(
    zonas.map(z => String(z.AMIE_NUCLEO || "").trim()).filter(a => a && a !== "SIN_NUCLEO")
  );

  const nucleosHuerfanos = zonas.filter(
    z => (Number(z.TOTAL_NUCLEOS) || 0) === 1 && (Number(z.TOTAL_SATELITES) || 0) === 0
  ).length;

  return {
    TOTAL_SATELITES_BASE,
    SATELITES_CUBIERTOS: satCubiertos,
    SATELITES_NO_CUBIERTOS: satNoCubiertos,
    COBERTURA_TOTAL: coberturaPct,
    TOTAL_NUCLEOS: nucleosSet.size,
    NUCLEOS_HUERFANOS: nucleosHuerfanos
  };
}

/* ---------- 3) DASHBOARD (sobrescribe updateStatistics) ---------- */
/* updateStatistics(stats) existe en el código original y es llamado por el flujo principal.
   Lo sobrescribimos para que SIEMPRE pinte valores espejo del mapa (sin tocar posiciones). */
function updateStatistics(stats) {
  const kpi = calcularKPIsDesdeMapa();
  const set = (id, val) => {
    const el = document.getElementById(id);
    if (el) el.textContent = (typeof val === 'number') ? val.toLocaleString() : String(val);
  };

  // Coherencia total con mapa
  set("totalNucleos", kpi.TOTAL_NUCLEOS);
  set("totalSatellites", kpi.TOTAL_SATELITES_BASE);
  set("coveragePercent", `${kpi.COBERTURA_TOTAL.toFixed(2)}%`);
  set("sinCobertura", kpi.SATELITES_NO_CUBIERTOS);

  // Estudiantes: conserva el valor calculado original si existe, pero no rompe si no está
  set("totalStudents", stats?.totalStudents ?? 0);

  // Núcleos activos: coherente con núcleos detectados en el mapa
  set("nucleosActivos", kpi.TOTAL_NUCLEOS);

  // Barra visual si existe
  const bar = document.getElementById("coverageBar");
  if (bar) bar.style.width = `${Math.min(100, kpi.COBERTURA_TOTAL)}%`;
}

/* ---------- 4) Cobertura_Zonas (sobrescribe buildCoverageZonesRowsForExport) ---------- */
function buildCoverageZonesRowsForExport() {
  // Mantiene la hoja Cobertura_Zonas como espejo del mapa
  return buildCoberturaZonasFromMapState().map(z => ({
    ZONA_ID: z.ZONA_ID,
    AMIE_NUCLEO: z.AMIE_NUCLEO,
    TOTAL_NUC: z.TOTAL_NUCLEOS,
    SATELLITES_FI: z.SATELITES_CUBIERTOS,
    TOTAL_SAT: z.TOTAL_SATELITES,
    CENTRO_LAT: z.CENTRO_LAT,
    CENTRO_LNG: z.CENTRO_LNG,
    ALIAS_MAPA: z.ALIAS_MAPA,
    TIPO_BUFFER: z.TIPO_BUFFER,
    BUFFER_MOVIDO: z.BUFFER_MOVIDO
  }));
}

/* ---------- 5) performSpatialJoin (sobrescribe para que el modal/export usen espejo) ---------- */
function performSpatialJoin() {
  const zonas = buildCoberturaZonasFromMapState();
  const kpi = calcularKPIsDesdeMapa();
  ...
  const __cb = (typeof customBuffers !== 'undefined') ? customBuffers : [];
  const allBuffers = [...__safeArr(__eb), ...__safeArr(__cb)];
  const byId = new Map(allBuffers.map((b, idx) => [String(b.id || `B${idx+1}`), b]));

  // Construir "buffers" mínimo para que el modal cuente correctamente y para trazabilidad
  const buffers = zonas.map(z => {
    const b = byId.get(String(z.ZONA_ID));
    return {
      id: z.ZONA_ID,
      isCustom: !!(b?.isCustom),
      alias: z.ALIAS_MAPA,
      tipo: z.TIPO_BUFFER,
      centerLat: z.CENTRO_LAT,
      centerLng: z.CENTRO_LNG,
      nucleoAmie: z.AMIE_NUCLEO,
      satellitesCovered: z.SATELITES_CUBIERTOS,
      satellitesTotal: z.TOTAL_SATELITES
    };
  });

  // Satélites fiscales "All": mantener mínimo para export (amie + nearestBuffer/orphanBufferId)
  const satellitesAllExport = [];
  const satAssignments = (typeof orphanBufferCoverageState !== 'undefined' && orphanBufferCoverageState && orphanBufferCoverageState.satAssignments) ? orphanBufferCoverageState.satAssignments : null;

  // Fuente preferida: globalData.satellites (si existe) para conservar metadata
  const baseSatList = (typeof globalData !== 'undefined' && globalData?.satellites && Array.isArray(globalData.satellites))
    ? globalData.satellites
    : [];

  const seen = new Set();

  // Helper para extraer "amie" de objetos variados
  const getAmie = (s) => String(s?.amie || s?.AMIE || s?.codigo || s?.CODAMIE || "").trim();

  // Primero, satélites del dataset
  baseSatList.forEach(s => {
    const amie = getAmie(s);
    if (!amie) return;
    seen.add(amie);

    const info = satAssignments?.get(amie);
    const bufferId = info?.bufferId || null;
    const covered = info?.covered === true;

    satellitesAllExport.push({
      amie,
      ...(typeof s === "object" ? s : {}),
      nearestBuffer: bufferId ? { kindName: "Buffer", bufferId, distanceKm: null } : null,
      orphanBufferId: bufferId || null,
      covered
    });
  });

  // Segundo, satélites que solo estén en asignaciones (por si no están en globalData)
  if (satAssignments?.forEach) {
    satAssignments.forEach((info, amie) => {
      const a = String(amie || "").trim();
  ...
  // totalAMIEs: núcleos únicos + satélites cubiertos (coherente con mapa)
  const nucleosSet = new Set(zonas.map(z => String(z.AMIE_NUCLEO || "").trim()).filter(a => a && a !== "SIN_NUCLEO"));
  const totalAMIEs = nucleosSet.size + kpi.SATELITES_CUBIERTOS;
  ...
  return {
    ...
    summary: {
      ...
      totalAMIEs,
      totalNucleos: nucleosSet.size,
      totalSatellites: kpi.TOTAL_SATELITES_BASE,
      totalStudents: 0,
      coveragePercent: kpi.COBERTURA_TOTAL.toFixed(2)
    },
    buffers,
    satellitesSummary: {
      total: satellitesAllExport.length,
