...
function analyzeOrphans() {
  ...
  satellites.forEach((sat, si) => {
    ...
    if (covered) {
      ...
      selected.forEach(ni => {
        ...
        const dist = haversineMeters(lat, lng, sat.lat, sat.lng);
        if (dist < minDist) {
          minDist = dist;
          closestNi = ni;
        }
      ...
      if (closestNi !== null) {
        orphanAnalysis.forcedAssignments.set(si, { ni: closestNi, distance: minDist });
        forcedCovered++;
      } else {
  ...
  selected.forEach(ni => {
    ...
    satCandidates.forEach((cands, si) => {
      if (cands) {
        cands.forEach(c => {
          if (c.ni === ni && c.dist <= BUFFER_RADIUS_M) hasSats = true;
        });
      }
    });
    
    orphanAnalysis.forcedAssignments.forEach(assign => {
      if (assign.ni === ni) hasSats = true;
    });
    
    if (!hasSats) orphanAnalysis.orphanNucleos.add(ni);
  });
...
function coverOrphansIntelligently() {
  if (!globalData || !globalData.satellites || !globalData.nucleos) {
    console.log("[SKIP] globalData no disponible para cobertura");
    return;
  }
  
  console.log("🎯 Analizando cobertura inteligente de huérfanos...");
  
  const { satellites, nucleos, satCandidates, selected } = globalData;
  let addedBuffers = 0;
  let newCoverage = 0;
  
  // Identificar satélites huérfanos que están CERCA de algún núcleo
  const nearOrphans = [];
  ...
  satellites.forEach((sat, si) => {
    ...
    if (!isCovered) {
      // Buscar núcleos cercanos (dentro de 7.5km)
      const nearbyNucleos = [];
      
      nucleos.forEach((nuc, ni) => {
        const dist = haversineMeters(nuc.lat, nuc.lng, sat.lat, sat.lng);
        if (dist <= BUFFER_RADIUS_M) {
          nearbyNucleos.push({ ni, dist });
        }
      });
      
      if (nearbyNucleos.length > 0) {
        // Ordenar por distancia
        nearbyNucleos.sort((a, b) => a.dist - b.dist);
        nearOrphans.push({
          si,
          sat,
          nearbyNucleos
  ...
  console.log(`📊 Huérfanos cercanos encontrados: ${nearOrphans.length}`);
  
  // Agrupar por núcleo más cercano
  const nucleoCandidates = new Map();
  
  nearOrphans.forEach(orphan => {
    const closestNi = orphan.nearbyNucleos[0].ni;
    if (!nucleoCandidates.has(closestNi)) {
      nucleoCandidates.set(closestNi, []);
    }
    nucleoCandidates.get(closestNi).push(orphan);
  });
  
  // Activar núcleos que pueden cubrir múltiples huérfanos
  const nucleosToActivate = [];
  nucleoCandidates.forEach((orphans, ni) => {
    // Solo activar si cubre al menos 3 satélites huérfanos
    if (orphans.length >= 3 && !selected.has(ni)) {
      nucleosToActivate.push({
        ni,
  ...
  // Ordenar por cantidad de huérfanos que cubre
  nucleosToActivate.sort((a, b) => b.count - a.count);
  
  console.log(`🎯 Núcleos candidatos para activar: ${nucleosToActivate.length}`);
  
  // Activar núcleos (respetando límite de buffers)
  const maxNewBuffers = Math.min(50, MAX_BUFFERS - selected.size);
  
  nucleosToActivate.slice(0, maxNewBuffers).forEach(candidate => {
    const nucleo = nucleos[candidate.ni];
    
    // Agregar a selected
    selected.add(candidate.ni);
    
    // Dibujar buffer
    const circle = L.circle([nucleo.lat, nucleo.lng], {
    ...
    circle.bindPopup(`
      <b>Buffer Activado (Cobertura Huérfanos)</b><br>
      Núcleo: ${nucleo.name}<br>
      Cubre: ${candidate.count} satélites huérfanos
    `);
    
    // Agregar a editableBuffers
    editableBuffers.set(candidate.ni, {
      circle: circle,
      originalPos: { lat: nucleo.lat, lng: nucleo.lng },
      currentPos: { lat: nucleo.lat, lng: nucleo.lng }
    });
    
    addedBuffers++;
    newCoverage += candidate.count;
  });
  
  console.log(`✅ Buffers agregados: ${addedBuffers}`);
  console.log(`✅ Nueva cobertura: ${newCoverage} satélites huérfanos`);
  
  // Actualizar globalData.selected
  globalData.selected = selected;
  
  // Re-analizar después de agregar buffers
  setTimeout(() => {
    analyzeOrphans();
  }, 500);
}
