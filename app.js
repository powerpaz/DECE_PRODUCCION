...
function exportToExcel() {
  ...
  // --- Registros administrativos (GIEE) ---
  // Fuente: dataset completo parseado desde el CSV (todas las filas válidas).
  // Estructura alineada al archivo "Resultados_prototipo.xlsx" (Hoja GIEE).
  const rawRows = window._rawRowsForExport || [];
  if (Array.isArray(rawRows) && rawRows.length) {
    const gHeaders = ['AMIE','DISTRITO','Sostenimiento','Jurisdicción','IE_Fiscales','Jornadas','Grupo_DECE','COD_GDECE','COORD_DECE','PO_ProfDECE','DECE NUCLEO','AMIE_NUCLEO'];

    const gRows = rawRows.map(r => ([
      String(r?.amie ?? '').trim(),
      String(r?.dist ?? '').trim(),
      String(r?.sostenimiento ?? '').trim(),
      String(r?.jurisdiccion ?? '').trim(),
      String(r?.ieFiscalesRaw ?? r?.ieFiscales ?? '').trim(),
      String(r?.jornadasRaw ?? r?.jornadas ?? '').trim(),
      String(r?.grupoDece ?? '').trim(),
      String(r?.codGDECE ?? '').trim(),
      String(r?.coordDeceRaw ?? r?.code ?? '').trim(),
      String(r?.profDeceRaw ?? r?.profDece ?? '').trim(),
      String(r?.deceNucleoRaw ?? r?.deceNucleo ?? '').trim(),
      String(r?.amieNucleo ?? '').trim()
    ]));

    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([gHeaders, ...gRows]), 'GIEE');
  }
...
function resolveColumnIndexes(headerRow) {
  ...
  return {
    idx: {
      ...
      dist: find(["distrito"]),
      jurisdiccion: find(["jurisdicción", "jurisdiccion"]),
      zona: find(["zona"]),
...
// pero NO forzamos el color a rojo: el color depende únicamente de estar dentro/fuera de cualquier buffer visible.
    ...
    const orphanFill = '#a371f7';
    const uncoveredFill = '#f85149';
    const marker = L.circleMarker([s.lat, s.lng], {
      radius: normalCovered ? 7 : 8,
      fillColor: orphanBid ? orphanFill : (normalCovered ? coveredFill : uncoveredFill),
  ...
  console.log('%c🔴 Satélites SIN cobertura: ' + rojosCount, 'color: #f85149; font-size: 14px; font-weight: bold;');
  console.log('%c📊 Total satélites: ' + satellites.length, 'color: #60a5fa; font-size: 14px;');
  console.log('%c═══════════════════════════════════', 'color: #9333ea; font-weight: bold;');
  
  if (rojosCount > 0) {
    console.log('%c✅ HAY ' + rojosCount + ' SATÉLITES ROJOS - Usa el toggle para verlos', 'background: #f85149; color: white; padding: 8px; font-weight: bold; font-size: 12px;');
  } else {
