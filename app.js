...
// ===============================
// BUSCADOR AMIE (Topbar: debajo de Guardar Cambios)
// ===============================
function amieNormalize(v){
  return String(v || "").trim().toUpperCase();
}

function amieFindLayer(amieCode){
  let found = null;
  ...
  map.eachLayer(function(layer){
    ...
    const code = p.AMIE || p.CODAMIE || p.CODIGO_INS || p.CODIGO || p.CODIGO_AMIE;
    if (code && amieNormalize(code) === amieCode){
      found = layer;
    }
  });
...
function amieZoomToLayer(layer){
  if (!layer) return;
...
function initAMIESearchTopbar(){
  ...
  const run = () => {
    ...
    if (!amie){
      if (msg) msg.textContent = "Ingrese un código AMIE. Ejemplo: 01H01581";
      return;
    }

    // Orientación (no bloquea)
    if (!/^[0-9]{2}[A-Z][0-9]{5}$/.test(amie)){
      if (msg) msg.textContent = "Formato sugerido: 01H01581 (2 dígitos + letra + 5 dígitos). Buscando…";
    } else {
      if (msg) msg.textContent = "Buscando…";
    }

    const layer = amieFindLayer(amie);
    if (!layer){
      if (msg) msg.textContent = "AMIE no encontrado en las capas cargadas.";
      return;
    }

    if (msg) msg.textContent = "AMIE encontrado. Centrando en el mapa…";
    amieZoomToLayer(layer);
    setTimeout(() => amieOpenPopup(layer), 250);
  };
  ...
  if (msg) msg.textContent = "Listo. Escriba el AMIE y presione Buscar.";
}

document.addEventListener("DOMContentLoaded", () => {
  // Espera por capas/DOM
  setTimeout(initAMIESearchTopbar, 800);
  setTimeout(initAMIESearchTopbar, 1800);
});
