...
function exportToExcel() {
  try {
    ...
    function buildMapExplainRow(r){
      ...
      const isSatRow = isSatelliteRow(r);
      ...
      return {
        ...
        DECE_NUCLEO: isSatRow ? ((deceN === '1' || deceN === '0') ? deceN : toDash(r?.deceNucleoRaw ?? r?.deceNucleo)) : '',
        AMIE_NUCLEO: isSatRow ? ((cat === 'SAT_HUERFANO') ? '' : ((cat === 'SAT_BUFFER_B' && orphanBid && deceN === '0') ? String(orphanAlias || orphanBid) : (amieN ? String(amieN).trim() : toDash(r?.amieNucleo)))) : '',
        DISTANCIA_KM: distKm,
    ...
    const rows = rawRows
      ...
      // FIX: Si el registro es NÚCLEO, no debe auto-asignarse como AMIE_NUCLEO (evita filas tipo AMIE_NUCLEO = AMIE del núcleo)
