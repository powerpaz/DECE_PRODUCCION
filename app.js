...
function buildNucleoLookup(nucleos, selectedSet) {
  ...
  return { all, selected, allByProv, allByProvCant, selByProv, selByProvCant };
}
...
function findClosestNucleoForOrphan(sat, lookup, preferSelected = true) {
  ...
  // 2) Resto del país: preferir misma provincia; si no existe, usar el pool general.
...
function mapRowsToData(rows, idx) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    data.push({ lat, lng, code: typeCode, codGDECE, name, dist, students, amie, provincia, codProvincia, canton, codCanton });
