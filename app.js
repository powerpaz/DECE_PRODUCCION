...
const ORPHAN_BUFFER_NUCLEO_MAX_M = 7500; // 7.5 km: máximo satélite→núcleo fiscal para atender buffers B# // 7 km: máximo satélite→núcleo (solo fiscales/seleccionados) para cubrir buffers B#
...
// Definición: buffer que contiene >=1 satélite y 0 núcleos dentro de 7.5km.
...
function getOrphanPopupInfo(si) {
  try {
    const bufferId = orphanBufferCoverageState?.satInsideOrphan?.get?.(si);
    if (!bufferId) return null;
    const alias = orphanBuffersState?.aliasById?.get?.(bufferId) || null;
    const assign = orphanBufferCoverageState?.satAssignments?.get?.(si) || null;
    if (!assign) return { alias, bufferId, assigned: null };
    const nuc = globalData?.nucleos?.[assign.ni] || null;
    return {
      alias,
      bufferId,
      assigned: {
        ni: assign.ni,
        name: nuc?.name || `Núcleo ${assign.ni}`,
        amie: nuc?.amie || '',
        sostenimiento: nuc?.sostenimiento || '',
        dist: nuc?.dist || '',
        distanceMeters: assign.distanceMeters
      }
    };
  } catch (e) {
    return null;
...
function getFiscalNucleoCandidates({ onlySelected = false } = {}) {
  if (!globalData) return [];
  const { nucleos, selected } = globalData;

  const candidates = [];
  const indices = onlySelected ? (selected || []) : nucleos.map((_, i) => i);

  (indices || []).forEach(ni => {
    const nuc = nucleos?.[ni];
    if (!nuc) return;
    if (!isFiscalInstitution(nuc)) return; // seguridad
...
function recomputeOrphanBufferCoverage() {
  ...
  const nucCandidates = getFiscalNucleoCandidates({ onlySelected: false });
  ...
  orphanBuffersState.list.forEach(ob => {
    ...
    // satélites dentro del buffer (7.5 km)
    const satsInBuffer = [];
    ...
    satsInBuffer.forEach(({ sat, si }) => {
      ...
      for (const n of nucCandidates) {
        const satDist = String(sat.dist || '').trim();
        const nucDist = String(n.dist || '').trim();
        if (satDist && nucDist && satDist !== nucDist) continue;
        const dist = haversineMeters(n.lat, n.lng, sat.lat, sat.lng);
        if (dist < bestDist) { bestDist = dist; best = n; }
      }

      if (best && Number.isFinite(bestDist) && bestDist <= ORPHAN_BUFFER_NUCLEO_MAX_M) {
        coveredSatellites++;
        orphanBufferCoverageState.satAssignments.set(si, { ni: best.ni, distanceMeters: bestDist, bufferId });

        if (!perNucleo.has(best.ni)) perNucleo.set(best.ni, { count: 0, sumDist: 0, maxDist: 0 });
...
function exportToExcel() {
  ...
  const summaryData = [['REPORTE DE ANÁLISIS ESPACIAL DECE'],['Fecha:', data.exportDate],[''],['MÉTRICAS'],
    ...
    ['Buffers huérfanos (B#):', (data.orphanBuffers?.length ?? 0)],
  ...
  const buffersHeaders = ['ID Buffer','Nombre','Tipo','Alias Huérfano (B#)','Núcleo AMIE','Núcleo Nombre','Núcleo Sostenimiento','Cobertura Satélites ≤7km','Satélites en buffer','Dist. prom (km)','Dist. máx (km)','Lat','Lng','Radio(m)','Fue Movido','Total AMIEs','Núcleos','Satélites','Estudiantes'];
  const buffersData = data.buffers.map(b => [
    b.bufferId,
    b.bufferName,
    b.isCustom ? 'Personalizado' : 'Original',
    b.orphanAlias || '',
    b.nucleoAmie || '',
    b.nucleoNombre || '',
    b.nucleoSostenimiento || '',
    b.coberturaSatellites7km ?? '',
    b.coberturaSatellitesTotal ?? '',
    b.coberturaAvgKm ?? '',
    b.coberturaMaxKm ?? '',
    b.centerLat,
    b.centerLng,
  ...
  const instHeaders = ['Buffer','Alias Huérfano','Núcleo AMIE','Núcleo Nombre','Núcleo Sostenimiento','AMIE','Nombre','Sostenimiento','Tipo','COD_GDECE','Lat','Lng','Distancia(m)','Distancia(km)','Estudiantes','Distrito'];
  const instData = [];
  data.buffers.forEach(buffer => buffer.institutions.forEach(inst => instData.push([
    buffer.bufferName,
    buffer.orphanAlias || '',
    buffer.nucleoAmie || '',
    buffer.nucleoNombre || '',
    buffer.nucleoSostenimiento || '',
    inst.amie,
    inst.name,
    inst.sostenimiento || '',
    inst.typeName,
  ...
  // --- Buffers huérfanos (B1, B2, ...) ---
  if (Array.isArray(data.orphanBuffers) && data.orphanBuffers.length) {
    const oHeaders = ['Alias','Buffer_ID','Lat Centro','Lng Centro','Satélites','Estudiantes','Núcleo fiscal (AMIE)','Núcleo fiscal (Nombre)','Sostenimiento núcleo','Satélites cubiertos ≤7km','Satélites en buffer','Dist. prom (km)','Dist. máx (km)'];
    const oRows = data.orphanBuffers.map(b => [b.alias, b.bufferId, b.centerLat, b.centerLng, b.satellitesCount, b.totalStudents, b.nucleoAmie || '', b.nucleoNombre || '', b.nucleoSostenimiento || '', b.coberturaSatellites7km ?? '', b.coberturaSatellitesTotal ?? '', b.coberturaAvgKm ?? '', b.coberturaMaxKm ?? '']);
    XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([oHeaders, ...oRows]), 'Buffers_Huérfanos');
  }

  // --- Satélites (uno por registro) ---
  if (Array.isArray(data.satellitesAll) && data.satellitesAll.length) {
    const satHeaders = ['AMIE','Nombre','Sostenimiento','Estudiantes','Distrito','Zona','Provincia','Cantón','Lat','Lng','¿Cubierto por buffer?','Buffer más cercano','Tipo buffer','Distancia a buffer (m)','Distancia a buffer (km)','¿Tiene conexión forzada?','Núcleo asignado','AMIE núcleo asignado','Dist. conexión forzada (m)','Dist. conexión forzada (km)','¿Extendida >7km?'];
    const satRows = data.satellitesAll.map(s => [
      s.amie, s.name, s.sostenimiento || '', s.students, s.distrito, s.zona ?? '', s.provincia, s.canton,
      s.lat, s.lng,
      s.covered ? 'Sí' : 'No',
  ...
  if (Array.isArray(data.orphanNucleos) && data.orphanNucleos.length) {
    const onHeaders = ['Índice','AMIE','Nombre','Sostenimiento','Distrito','Lat','Lng'];
    const onRows = data.orphanNucleos.map(n => [n.ni, n.amie, n.name, n.sostenimiento || '', n.distrito, n.lat, n.lng]);
...
function showBufferPopup(bufferData, isCustom = false) {
  ...
  const coverageTag = (isCustom && bufferData?.orphanAlias)
    ? (cov && cov.bestNi != null && cov.coveredSatellites > 0
        ? `<div class="popup-row"><span class="popup-label">Cobertura fiscal:</span><span class="popup-value" style="color:#58a6ff;font-weight:800">${escapeHTML(cov.nucleoNombre || '')} (${escapeHTML(cov.nucleoAmie || '')})</span></div><div class="popup-row"><span class="popup-label">Satélites cubiertos:</span><span class="popup-value">${cov.coveredSatellites}/${cov.totalSatellites} (≤ 7 km)</span></div>`
        : `<div class="popup-row"><span class="popup-label">Cobertura fiscal:</span><span class="popup-value" style="color:#f85149;font-weight:800">SIN COBERTURA (≤ 7 km)</span></div>`)
    : '';
...
function drawForcedConnections(targetLinesArray = null) {
  ...
  orphanAnalysis.forcedAssignments.forEach((assign, si) => {
    // Si el satélite pertenece a un buffer huérfano B#, su atención se maneja por asignación fiscal (no por 'forzada').
    if (orphanBufferCoverageState?.satInsideOrphan?.get?.(si)) return;
...
// Conexiones para buffers huérfanos B#: satélite → núcleo fiscal asignado (≤ 7.5 km y mismo distrito)
function drawOrphanFiscalAssignments(targetLinesArray = null) {
  if (!globalData || !orphanBufferCoverageState?.satAssignments?.size) return;
  const { satellites, nucleos } = globalData;

  orphanBufferCoverageState.satAssignments.forEach((assign, si) => {
    const sat = satellites?.[si];
    const nuc = nucleos?.[assign.ni];
    if (!sat || !nuc) return;

    const pos = getCurrentNucleoLatLng(assign.ni, nuc);
    const alias = orphanBuffersState?.aliasById?.get?.(assign.bufferId) || '';

    const line = L.polyline(
      [[sat.lat, sat.lng], [pos.lat, pos.lng]],
      {
        color: '#f59e0b',
        weight: 2,
        opacity: 0.8,
    ...
    line.bindPopup(`
      <b>Atención por núcleo fiscal (Buffer ${escapeHTML(alias)})</b><br>
      Satélite: ${escapeHTML(sat.name)}<br>
      Sost. satélite: ${escapeHTML(sat.sostenimiento || '')}<br>
      → Núcleo: ${escapeHTML(nuc.name)}<br>
      AMIE núcleo: ${escapeHTML(nuc.amie || '')}<br>
      Sost. núcleo: ${escapeHTML(nuc.sostenimiento || '')}<br>
      Distancia: ${(assign.distanceMeters / 1000).toFixed(2)} km
    `);
...
function regenerateAnimations() {
  ...
  satellites.forEach((sat, si) => {
    ...
    if (bestBuffer) {
      const orphanBid = orphanBufferCoverageState?.satInsideOrphan?.get?.(si);
      const assign = orphanBid ? orphanBufferCoverageState?.satAssignments?.get?.(si) : null;

      // Si está en buffer B# y tiene núcleo fiscal asignado, la conexión la dibuja drawOrphanFiscalAssignments()
      if (orphanBid && assign) return;
  ...
  // Incluir conexiones para buffers huérfanos B# (asignación fiscal)
  drawOrphanFiscalAssignments(animationLines);
...
function analyzeOrphans() {
  ...
  // (Nuevo) Asignar B1..Bn a buffers huérfanos y mostrar etiquetas en mapa
  updateOrphanBuffers();
}
...
function drawSatellites(satellites, satCandidates, selected) {
  ...
  satellites.forEach((s, si) => {
    ...
    // (Nuevo) Si el satélite está dentro de un buffer huérfano B#, la cobertura se define por núcleo fiscal seleccionado (≤ 7 km)
    const orphanBid = orphanBufferCoverageState?.satInsideOrphan?.get?.(si);
    if (orphanBid) {
      const assign = orphanBufferCoverageState?.satAssignments?.get?.(si);
      normalCovered = !!assign;
      normalDist = assign ? assign.distanceMeters : null;
    }
    ...
    const marker = L.circleMarker([s.lat, s.lng], {
      // Mantener TODOS los satélites en rojo, estén o no dentro de un buffer
      radius: 7,
      fillColor: '#f85149',
      color: '#7f1d1d',
      weight: 2,
...
marker.bindPopup(createSatellitePopup(s, { normalCovered, normalDist, forced, orphan: getOrphanPopupInfo(si) }));
...
function createNucleoPopup(n, satCount, totalStudents) {
  return `    <div class="popup-title">🏛️ Núcleo DECE</div>    <div class="popup-content">      <div class="popup-row"><span class="popup-label">AMIE:</span><span class="popup-value">${escapeHTML(n.amie || '')}</span></div>      <div class="popup-row"><span class="popup-label">Institución:</span><span class="popup-value">${escapeHTML(n.name || '')}</span></div>      <div class="popup-row"><span class="popup-label">Sostenimiento:</span><span class="popup-value">${escapeHTML(n.sostenimiento || '')}</span></div>      <div class="popup-row"><span class="popup-label">Distrito:</span><span class="popup-value">${escapeHTML(n.dist || '')}</span></div>      <div class="popup-row"><span class="popup-label">Estudiantes:</span><span class="popup-value" style="color:#d29922">${(n.students || 0).toLocaleString()}</span></div>    </div>`;
}

function createSatellitePopup(s, info) {
  // info: { normalCovered: boolean, normalDist: number|null, forced: {ni, distance}|null, orphan: {alias, bufferId, assigned}|null }
  ...
  const orphan = info && info.orphan ? info.orphan : null;

  // Estado principal
  let statusColor = normalCovered ? "#10b981" : "#dc2626";
  let statusText = normalCovered ? "✅ ATENDIDA" : "❌ SIN ATENCIÓN";

  // Si viene de un buffer huérfano B#, mostramos estado específico
  if (orphan && orphan.alias) {
    if (orphan.assigned) {
      statusColor = "#f59e0b";
      statusText = `🟡 ATENDIDA por Núcleo Fiscal (Buffer ${escapeHTML(orphan.alias)})`;
    } else {
      statusColor = "#dc2626";
      statusText = `🔴 SIN ATENCIÓN (Buffer ${escapeHTML(orphan.alias)} sin núcleo fiscal ≤ 7.5 km)`;
    }
  }

  const distLine = (normalDist !== null)
    ? `<div class="popup-row"><span class="popup-label">Distancia:</span><span class="popup-value">${(normalDist/1000).toFixed(2)} km</span></div>`
    : '';

  // Info asignación por B#
  let orphanLine = '';
  if (orphan && orphan.alias) {
    orphanLine = `<div class="popup-row"><span class="popup-label">Grupo:</span><span class="popup-value">${escapeHTML(orphan.alias)}</span></div>`;
    if (orphan.assigned) {
      const n = orphan.assigned;
      orphanLine += `
        <div class="popup-row"><span class="popup-label">Núcleo asignado:</span><span class="popup-value">${escapeHTML(n.name || '')}</span></div>
        <div class="popup-row"><span class="popup-label">AMIE núcleo:</span><span class="popup-value">${escapeHTML(n.amie || '')}</span></div>
        <div class="popup-row"><span class="popup-label">Sost. núcleo:</span><span class="popup-value">${escapeHTML(n.sostenimiento || '')}</span></div>
        <div class="popup-row"><span class="popup-label">Dist. núcleo:</span><span class="popup-value">${escapeHTML(n.dist || '')}</span></div>
        <div class="popup-row"><span class="popup-label">Distancia núcleo:</span><span class="popup-value">${(n.distanceMeters/1000).toFixed(2)} km</span></div>
      `;
    }
  }

  // Info asignación distrital (si existe en otras rutinas)
  const forcedLine = (!normalCovered && forced && !orphan)
    ? `<div class="popup-row"><span class="popup-label">Asignación:</span><span class="popup-value" style="color:#f59e0b">↪ Atención distrital (${(forced.distance/1000).toFixed(2)} km)</span></div>`
    : '';
  ...
  return `<div class="popup-title">📍 Satélite ${normalCovered ? '🟢' : '🔴'}</div>` +
    `<div class="popup-content">` +
      ...
      `<div class="popup-row"><span class="popup-label">Sostenimiento:</span><span class="popup-value">${escapeHTML(s.sostenimiento || '')}</span></div>` +
...
function computeStatistics(nucleos, satellites, satCandidates, selected, nucleoStats) {
  ...
  satellites.forEach((s, si) => {
    ...
    // Cobertura por buffers actuales (≤ 7.5 km)
    const hit = findClosestInGridWithin(activeGrid, s.lat, s.lng, BUFFER_RADIUS_M, 1);
    if (hit) isCovered = true;

    // Si está dentro de un buffer huérfano B#, la cobertura se define por asignación a núcleo fiscal (≤ 7.5 km)
    const orphanBid = orphanBufferCoverageState?.satInsideOrphan?.get?.(si);
    if (orphanBid) {
      isCovered = !!orphanBufferCoverageState?.satAssignments?.get?.(si);
    }
