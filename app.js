...
function drawSatellites(satellites, satCandidates, selected) {
  ...
  satellites.forEach((s, si) => {
    ...
    // 2) Si es huérfano y tiene asignación de apoyo, marcar como cubierto
    ...
    // 🟢 VERDE para satélites CON cobertura (tamaño 5)
    // 🔴 ROJO FUERTE para satélites SIN cobertura (tamaño 7 - MÁS GRANDE)
    const marker = L.circleMarker([s.lat, s.lng], {
      radius: isCovered ? 5 : 7,  // Rojos más grandes
      fillColor: isCovered ? '#10b981' : '#dc2626',
      color: isCovered ? '#fff' : '#7f1d1d',  // Borde rojo oscuro para rojos
      weight: isCovered ? 1 : 2,  // Borde más grueso para rojos
      opacity: 1,
      fillOpacity: isCovered ? 0.85 : 0.95,  // Rojos más opacos
      renderer: canvasRenderer,
      className: isCovered ? 'satellite-covered' : 'satellite-uncovered'
    });
  ...
  console.log(`[SATELLITES] 🟢 Verdes (cubiertos): ${coveredCount}`);
  console.log(`[SATELLITES] 🔴 Rojos (sin cobertura): ${uncoveredCount}`);
  console.log(`[SATELLITES] Total: ${satellites.length}`);
}
...
function createSatellitePopup(s, distMetersOrNull) {
  ...
  // 🟢 VERDE para cubierto, 🔴 ROJO para sin cobertura
  return `<div class="popup-title">📍 Satélite ${covered ? '🟢' : '🔴'}</div><div class="popup-content"><div class="popup-row"><span class="popup-label">Institución:</span><span class="popup-value">${escapeHTML(s.name)}</span></div><div class="popup-row"><span class="popup-label">Distrito:</span><span class="popup-value">${escapeHTML(s.dist)}</span></div><div class="popup-row"><span class="popup-label">Estado:</span><span class="popup-value" style="color:${covered ? "#10b981" : "#dc2626"}">${covered ? "✅ CON cobertura" : "❌ SIN cobertura"}</span></div>${covered ? `<div class="popup-row"><span class="popup-label">Distancia:</span><span class="popup-value">${(distMetersOrNull/1000).toFixed(2)} km</span></div>` : ''}<div class="popup-row"><span class="popup-label">Estudiantes:</span><span class="popup-value" style="color:#d29922">${(s.students || 0).toLocaleString()}</span></div></div>`;
}
