...
// ========== BARRIDO INTELIGENTE ==========
function selected {
  console.log("🧹 BARRIDO INTELIGENTE: Limpiando buffers sin núcleos...");
  
  const buffersToRemove = [];
  
  selected.forEach(ni => {
    // Verificar si este buffer tiene un núcleo dentro
    const nucleo = nucleos[ni];
    if (!nucleo) {
      buffersToRemove.push(ni);
      return;
    }
    
    // Verificar que el núcleo esté dentro del radio del buffer
    const bufferData = editableBuffers.get(ni);
    const bufferLat = bufferData?.currentPos?.lat || nucleo.lat;
    const bufferLng = bufferData?.currentPos?.lng || nucleo.lng;
    
    const distToNucleo = haversineMeters(bufferLat, bufferLng, nucleo.lat, nucleo.lng);
    
    // Si el núcleo está fuera del buffer (solo hay satélites), eliminar
    if (distToNucleo > BUFFER_RADIUS_M) {
      buffersToRemove.push(ni);
    }
  });
  
  // Remover buffers sin núcleos
  buffersToRemove.forEach(ni => {
    selected.delete(ni);
  });
  
  console.log(`❌ Buffers eliminados (sin núcleos): ${buffersToRemove.length}`);
  console.log(`✅ Buffers mantenidos: ${selected.size}`);
  
  return selected;
}

// ========== COBERTURA INTELIGENTE DE HUÉRFANOS ==========
function // smartOrphanCoverage removed // selected) {
  console.log("🎯 COBERTURA INTELIGENTE: Activando núcleos para huérfanos...");
  
  let orphansFound = 0;
  let orphansCovered = 0;
  const nucleosToActivate = new Map(); // ni -> count de huérfanos
  
  // Identificar satélites huérfanos
  satellites.forEach((sat, si) => {
    let isCovered = false;
    
    // Verificar si ya está cubierto
    if (satCandidates[si]) {
      satCandidates[si].forEach(c => {
        if (selected.has(c.ni) && c.dist <= BUFFER_RADIUS_M) {
          isCovered = true;
    ...
    if (!isCovered) {
      ...
      // Buscar núcleo más cercano (≤7.5km)
      let closestNi = null;
      let minDist = BUFFER_RADIUS_M; // Solo considerar dentro del radio
      
      nucleos.forEach((nuc, ni) => {
        const dist = haversineMeters(nuc.lat, nuc.lng, sat.lat, sat.lng);
        if (dist <= minDist) {
          minDist = dist;
          closestNi = ni;
        }
      ...
      if (closestNi !== null) {
        if (!nucleosToActivate.has(closestNi)) {
          nucleosToActivate.set(closestNi, 0);
        }
        nucleosToActivate.set(closestNi, nucleosToActivate.get(closestNi) + 1);
      }
    }
  });
  
  // Activar núcleos que cubren huérfanos
  nucleosToActivate.forEach((count, ni) => {
    if (!selected.has(ni)) {
      selected.add(ni);
      orphansCovered += count;
    }
  });
  
  console.log(`📊 Huérfanos encontrados: ${orphansFound}`);
  console.log(`✅ Núcleos activados: ${nucleosToActivate.size}`);
  console.log(`✅ Huérfanos cubiertos: ${orphansCovered}`);
...
function computeCoverage(satCandidates, selected) {
  let covered = 0;
  satCandidates.forEach(cands => {
    if (cands) {
      for (let c of cands) {
  ...
  return covered / satCandidates.length;
}
...
async function loadBufferPositionsFromJSON() {
  try {
    const response = await fetch('dece-buffers-2025-12-23_1_.json');
    if (!response.ok) {
      console.log("⚠️ JSON no encontrado, usando posiciones por defecto");
      return null;
    }
    const data = await response.json();
    console.log("✅ JSON cargado:", data.editableBuffers.length, "posiciones");
    return data;
  } catch (err) {
...
// ========== APLICAR POSICIONES DEL JSON ==========
function applyBufferPositionsFromJSON(jsonData, nucleos, selected) {
  if (!jsonData || !jsonData.editableBuffers) return;
  
  console.log("📍 Aplicando posiciones desde JSON...");
  
  jsonData.editableBuffers.forEach(bufferData => {
    const ni = bufferData.ni;
    if (!selected.has(ni)) {
      selected.add(ni);
    }
    
    // Guardar posición del JSON
    const nucleo = nucleos[ni];
    if (nucleo) {
      const bufferInfo = editableBuffers.get(ni);
      if (bufferInfo) {
        bufferInfo.currentPos = {
          lat: bufferData.currentLat,
          lng: bufferData.currentLng
        };
        bufferInfo.originalPos = {
          lat: bufferData.originalLat,
          lng: bufferData.originalLng
        };
      }
    }
  });
  
  console.log("✅ Posiciones aplicadas desde JSON");
}
...
async function processData(data) {
  // Cargar posiciones desde JSON
  const jsonData = await loadBufferPositionsFromJSON();
...
if (!nucleos.length || !satellites.length) {
  ...
  // Dibujar conexiones para huérfanos
  setTimeout(() => {
    console.log("🔗 Iniciando dibujo de conexiones...");
    drawOrphanConnections(nucleos, satellites, satCandidates, result.selected);
  }, 1000);
  ...
  // Aplicar posiciones desde JSON (si existe)
  if (jsonData) {
    applyBufferPositionsFromJSON(jsonData, nucleos, result.selected);
  }
...
function setCoverGreedy(nucleos, satellites, satCandidates) {
  ...
  // OPTIMIZACIÓN 1: Barrido inteligente (eliminar buffers sin núcleos)
  const cleanedSelected = selected;
  
  // OPTIMIZACIÓN 2: Cobertura de huérfanos (activar núcleos cercanos)
  const optimizedSelected = // smartOrphanCoverage removed // cleanedSelected);
  
  // Recalcular uncovered después de optimización
  const finalUncovered = new Set();
  satellites.forEach((sat, si) => {
    let covered = false;
    if (satCandidates[si]) {
      for (let c of satCandidates[si]) {
        if (optimizedSelected.has(c.ni) && c.dist <= BUFFER_RADIUS_M) {
          covered = true;
          break;
        }
      }
    }
    if (!covered) finalUncovered.add(si);
  });
  
  console.log(`📊 Cobertura final: ${((1 - finalUncovered.size / satellites.length) * 100).toFixed(2)}%`);
  
  return { selected: optimizedSelected, uncovered: finalUncovered };
}
