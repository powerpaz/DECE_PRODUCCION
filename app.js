...
function performSpatialJoin() {
  ...
  return {
    ...
    orphanBuffers: orphanBuffersState?.list || []
  };
}
...
function exportToExcel() {
  ...
  const buffersHeaders = ['ID Buffer','Nombre','Tipo','Alias Huérfano','Núcleo AMIE','Núcleo Nombre','Núcleo Sostenimiento','Lat Centro','Lng Centro','Radio (m)','Fue Movido','Total AMIEs','Núcleos','Satélites','Estudiantes'];
  ...
  if (Array.isArray(data.orphanBuffers) && data.orphanBuffers.length) {
    const oHeaders = ['Alias','Buffer_ID','Lat Centro','Lng Centro','Satélites','Estudiantes'];
    const oRows = data.orphanBuffers.map(b => [b.alias, b.bufferId, b.centerLat, b.centerLng, b.satellitesCount, b.totalStudents]);
