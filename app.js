...
function resolveColumnIndexes(headerRow) {
  ...
  return {
    idx: {
      ...
      profDece: find(["profesionales", "prof_dece", "prof dece", "profesional_dece", "n_profesionales", "dece_prof"])
    },
    issues: []
  };
}
...
function mapRowsToData(rows, idx) {
  ...
  for (let i = 1; i < rows.length; i++) {
    ...
    data.push({ lat, lng, code: typeCode, codGDECE, name, dist, zona, students, amie, provincia, codProvincia, canton, codCanton, sostenimiento, ieFiscales, grupoDece, profDece });
