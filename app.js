...
function showExportModal() {
  // Exportación directa (evita modal pesado que puede colgar GitHub Pages)
  try {
    window._exportData = performSpatialJoin();
    if (!window._exportData) {
      showNotification("❌ No hay datos para exportar", "error");
      return;
    }
    exportToExcel(); // hoja Registros_GIEE (Resultados_prototipo)
  } catch (e) {
    console.error(e);
    showNotification("❌ Error al exportar", "error");
  }
}
biertos / sin cobertura / conexiones extendidas)
  // ============================
...
function exportToExcel() {
  ...
  if (!data) { showNotification("❌ No hay datos para exportar", "error"); return; }

  showNotification("📊 Generando Excel...", "info");

  // Asignaciones dinámicas según el estado actual del mapa (núcleo asignado / cobertura)
  const asg = buildGIEEAssignmentsForExport();
  const deceMap = asg?.deceNucleoByAmie || new Map();
  const nucleoMap = asg?.amieNucleoByAmie || new Map();

  const rawRows = window._rawRowsForExport || [];
  const headers = [
    "AMIE","DISTRITO","Sostenimiento","Jurisdicción","IE_Fiscales","Jornadas",
    "Grupo_DECE","COD_GDECE","COORD_DECE","PO_ProfDECE","DECE NUCLEO","AMIE_NUCLEO"
  ];

  const toDash = (v) => (v === null || v === undefined || String(v).trim() === "") ? "-" : v;

  const rows = Array.isArray(rawRows) ? rawRows.map(r => {
    const amie = String(r?.amie || "").trim();
    const deceFlag = deceMap.get(amie) || "0";
    const deceNucleo = deceFlag === "1" ? "1" : "-";
    const amieNucleo = nucleoMap.get(amie) || toDash(r?.amieNucleo);

    return [
      toDash(amie),
      toDash(r?.dist),
      toDash(r?.sostenimiento),
      toDash(r?.jurisdiccion),
      toDash(r?.ieFiscalesRaw ?? (r?.ieFiscales ? "1" : "0")),
      toDash(r?.jornadas),
      toDash(r?.grupoDece),
      toDash(r?.codGDECE),
      toDash(r?.coordDece),
      toDash(r?.profDeceRaw ?? r?.profDece),
      deceNucleo,
      toDash(amieNucleo)
    ];
  }) : [];
  ...
  const fname = `Resultados_DECE_${formatDateForFilename()}.xlsx`;
...
function showBufferPopup(bufferData, isCustom = false) {
  ...
  const coverageTag = (isCustom && bufferData?.orphanAlias)
    ? (cov && cov.bestNi != null && cov.coveredSatellites > 0
        ...
        : `<div class="popup-row"><span class="popup-label">Cobertura fiscal:</span><span class="popup-value" style="color:#dc2626;font-weight:800">SIN COBERTURA (≤ 7.5 km)</span></div>`)
    : '';
...
function resolveColumnIndexes(headerRow) {
  ...
  return {
    idx: {
      ...
      // PO_ProfDECE (profesionales DECE)
      profDece: idxPoProfDece >= 0 ? idxPoProfDece : find(["profdece", "prof dece", "profesional_dece", "n_profesionales", "dece_prof"]),
...
function drawNucleos(nucleos, selected) {
  nucleos.forEach((n, ni) => {
    ...
    const nucleoStyle = getSostenimientoStyle('FISCAL');
...
// pero NO forzamos el color a rojo: el color depende únicamente de estar dentro/fuera de cualquier buffer visible.
    ...
    // Satélites (FISCAL 51–120) — coherente con Dashboard/Leyenda:
    // - CON cobertura (≤ 7.5 km): rojo con borde blanco
    // - SIN cobertura (> 7.5 km): rojo con borde punteado granate
    // - En Buffers B#: púrpura
    const fiscalFill = '#dc2626';
    const orphanFill = '#9333ea';

    const fill = orphanBid ? orphanFill : fiscalFill;
    const stroke = normalCovered
      ? '#ffffff'
      : (orphanBid ? '#5b21b6' : '#7f1d1d');

    const dash = (normalCovered || orphanBid) ? null : '5,6';

    const marker = L.circleMarker([s.lat, s.lng], {
      radius: (normalCovered || orphanBid) ? 7 : 8,
      fillColor: fill,
      color: stroke,
      weight: (normalCovered || orphanBid) ? 2 : 3,
      dashArray: dash,
      opacity: 1,
      fillOpacity: (normalCovered || orphanBid) ? 0.95 : 0.85,
      renderer: canvasRenderer
    });
