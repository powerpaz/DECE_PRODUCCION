...
function buildCoberturaZonasFromMapState() {
  const rows = [];
  const allBuffers = [...__safeArr(editableBuffers), ...__safeArr(customBuffers)];
  ...
  allBuffers.forEach((buffer, idx) => {
    ...
    __safeMap(orphanBufferCoverageState && orphanBufferCoverageState.satAssignments)?.forEach((info, satId) => {
    ...
    __safeList(satInsideOrphan).forEach(sat => {
...
function performSpatialJoin() {
  ...
  const satAssignments = (orphanBufferCoverageState && orphanBufferCoverageState.satAssignments) ? orphanBufferCoverageState.satAssignments : null;
