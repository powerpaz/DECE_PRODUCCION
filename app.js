...
// ==================== DEBUG FLAG (solo UI) ====================
// Activa modo debug de forma oculta con:
//  - URL: ?debug=1
//  - localStorage: DECE_DEBUG=1
// NOTA: Este flag SOLO controla elementos visuales (no altera reglas ni cálculos).
const DEBUG_MODE = (() => {
  try {
    const p = new URLSearchParams(window.location.search);
    if (p.get('debug') === '1') return true;
    return window.localStorage?.getItem('DECE_DEBUG') === '1';
  } catch (_) {
    return false;
  }
})();
...
if (DEBUG_MODE) {
  try { document.documentElement.classList.add('debug-enabled'); } catch (_) {}
}
...
function isEligibleSatellite(sat){
  // Universo oficial del modelo para KPIs/exportación:
  // Satélites FISCAL del grupo 51–120 (base = 1.415)
  return isFiscalInstitution(sat) && isSatellite51to120(sat) && !isExcludedGroup1to50(sat);
}
...
let districtsSpatialIndex = null; // Map gridKey -> [featureIdx]
let districtsFeatures = [];       // features normalizados

function _bboxOfCoords(coords, bbox) {
  // coords puede ser [lng,lat] o arreglo anidado
  ...
  for (const c of coords) _bboxOfCoords(c, bbox);
  return bbox;
}
...
function pointInPolygonLngLat(lng, lat, geom) {
  ...
  if (t === 'MultiPolygon') {
    for (const poly of c) {
      if (pointInPolygonLngLat(lng, lat, { type: 'Polygon', coordinates: poly })) return true;
    }
    return false;
  }
...
function buildDistrictSpatialIndex(geojson) {
  // Índice en grilla para acelerar point-in-polygon: cada distrito entra en celdas por bbox
  const grid = new Map();
  const feats = [];
  ...
  for (let i = 0; i < features.length; i++) {
    const f = features[i];
    const code = normalizeDistrictCode(f?.properties?.DA_DIST || f?.properties?.distrito || f?.properties?.DIST || '');
    const bbox = _bboxOfCoords(f?.geometry?.coordinates, [Infinity, Infinity, -Infinity, -Infinity]);
    if (!Number.isFinite(bbox[0])) continue;
    feats.push({ idx: i, code, bbox, geom: f.geometry, props: f.properties || {} });
  }

  // Grilla usando GRID_CELL_DEG
  feats.forEach((f, localIdx) => {
    const [minX, minY, maxX, maxY] = f.bbox;
    const minCLng = Math.floor(minX / GRID_CELL_DEG);
    const maxCLng = Math.floor(maxX / GRID_CELL_DEG);
    const minCLat = Math.floor(minY / GRID_CELL_DEG);
    const maxCLat = Math.floor(maxY / GRID_CELL_DEG);
    for (let clat = minCLat; clat <= maxCLat; clat++) {
      for (let clng = minCLng; clng <= maxCLng; clng++) {
        const key = `${clat},${clng}`;
        if (!grid.has(key)) grid.set(key, []);
        grid.get(key).push(localIdx);
      }
    }
  });
...
function getDistrictCodeForPoint(lat, lng) {
  // Devuelve código de distrito (ej. 17D05) si la capa está disponible.
  if (!districtsSpatialIndex || !districtsFeatures?.length) return '';
  const cellLat = Math.floor(lat / GRID_CELL_DEG);
  const cellLng = Math.floor(lng / GRID_CELL_DEG);
  const arr = districtsSpatialIndex.get(`${cellLat},${cellLng}`);
  if (!arr || !arr.length) return '';
  const x = lng, y = lat;
  for (const localIdx of arr) {
    const f = districtsFeatures[localIdx];
    const [minX, minY, maxX, maxY] = f.bbox;
    if (x < minX || x > maxX || y < minY || y > maxY) continue;
    if (pointInPolygonLngLat(x, y, f.geom)) return f.code || '';
  }
  return '';
}

function getRowDistrictCode(row) {
  // Prioriza distrito por geometría si está disponible; fallback a atributo del CSV
  const g = row?.distGeo || '';
  const a = row?.dist || row?.distrito || '';
  return normalizeDistrictCode(g || a);
}
...
// ==================== DISTRITOS (LÍMITE DISTRITAL) ====================
// Regla: Un núcleo fiscal SOLO puede cubrir/atender satélites fiscales dentro del MISMO distrito.
// Si un buffer atraviesa el límite distrital, los satélites del otro distrito NO se asignan a ese núcleo.
// En el reporte se marca el conflicto, y esos satélites pasan a “SIN COBERTURA” (y/o a buffers B# si corresponde).
...
// ==================== TRASH UI (Eliminar buffers por arrastre) ====================
let bufferTrashEl = null;
function getBufferTrashEl() {
  if (bufferTrashEl) return bufferTrashEl;
  bufferTrashEl = document.getElementById('bufferTrash');
  return bufferTrashEl;
}

function showBufferTrash(show) {
  const el = getBufferTrashEl();
  if (!el) return;
...
function setBufferTrashActive(active) {
  const el = getBufferTrashEl();
  if (!el) return;
...
function getSelectedFiscalNucleoCandidates() {
  ...
  (selected || []).forEach(ni => {
    ...
    candidates.push({
      ...
      lat: pos.lat,
      lng: pos.lng,
      distCode: String(nuc?.distCode || getRowDistrictCode(nuc) || '').trim()
    });
  });
  return candidates;
}
...
function recomputeOrphanBufferCoverage() {
  ...
  orphanBuffersState.list.forEach(ob => {
    ...
    satsInBuffer.forEach(({ sat, si }) => {
      const sDist = String(sat?.distCode || getRowDistrictCode(sat) || '').trim();
      let best = null;
      let bestDist = Infinity;

      for (const n of nucCandidates) {
        const nDist = String(n?.distCode || '').trim();
        // Regla: no mezclar distritos (si ambos existen)
        if (sDist && nDist && sDist !== nDist) continue;
...
function normalizeImportedState(raw) {
  // Acepta:
  // - Formato actual: {editableBuffers: [{ni,currentLat,currentLng,...}], customBuffers:[{id,lat,lng,name,...}]}
  // - Formatos legacy donde editableBuffers/customBuffers pueden venir como objetos.
  const state = raw && typeof raw === 'object' ? raw : {};
  const out = {
    editableBuffers: [],
    customBuffers: [],
    timestamp: state.timestamp || new Date().toISOString(),
    version: state.version || state.v || 'import'
  };

  const eb = state.editableBuffers;
  if (Array.isArray(eb)) {
    out.editableBuffers = eb;
  } else if (eb && typeof eb === 'object') {
    // map-like: { "12": {currentLat,...}, ...}
    out.editableBuffers = Object.keys(eb).map(k => ({ ni: isNaN(Number(k)) ? k : Number(k), ...eb[k] }));
  }

  const cb = state.customBuffers;
  if (Array.isArray(cb)) {
    out.customBuffers = cb;
  } else if (cb && typeof cb === 'object') {
    out.customBuffers = Object.keys(cb).map(k => ({ id: k, ...cb[k] }));
  }

  // Sanitizar + validar coordenadas
  out.editableBuffers = (out.editableBuffers || []).filter(b => validateBufferCoordinates(Number(b.currentLat), Number(b.currentLng)));
  out.customBuffers = (out.customBuffers || []).filter(b => validateBufferCoordinates(Number(b.lat), Number(b.lng)));

  return out;
}

function applyImportedStateToMap(state) {
  // Aplica posiciones al mapa SIN recargar (para que el % sea idéntico en cualquier máquina)
  if (!state) return false;

  // Si aún no se crearon buffers, no hay nada que mover en vivo
  if (!editableBuffers || editableBuffers.size === 0) return false;

  // 1) Mover buffers editables existentes
  const movedEditable = new Set();
  (state.editableBuffers || []).forEach(s => {
    const ni = s.ni;
    const data = editableBuffers.get(ni);
    if (!data || !data.circle) return;
    const lat = Number(s.currentLat), lng = Number(s.currentLng);
    if (!validateBufferCoordinates(lat, lng)) return;
    data.circle.setLatLng([lat, lng]);
    data.currentPos = { lat, lng };
    movedEditable.add(ni);
  });

  // 2) Re-crear buffers personalizados desde cero (evita duplicados/colisiones)
  try {
    for (const buf of (customBuffers || [])) {
      if (buf?.circle) layers.buffers.removeLayer(buf.circle);
    }
  } catch (_) {}
  customBuffers = [];

  const usedIds = new Set();
  (state.customBuffers || []).forEach((s, idx) => {
    let id = (s.id || `custom_${idx}`).toString();
    if (usedIds.has(id)) id = `${id}_${idx}`;
    usedIds.add(id);
  ...
  // 3) Recalcular análisis + dashboard/export (fuente única de verdad)
  if (globalData) {
    try {
      analyzeOrphans(); // incluye B# + refresca satélites y exportación
      const stats = computeStatistics(globalData.nucleos, globalData.satellites, globalData.satCandidates, globalData.selected, []);
      updateStatistics(stats);
    } catch (e) {
      console.warn("⚠️ Error recalculando tras importar:", e);
    }
  }

  // 4) Marcar como cambios pendientes
  try { markAsChanged(); } catch(_) {}
  return true;
}

/**
 * Importa el estado desde un archivo JSON (robusto + consistente entre equipos)
 */
function importBuffersState(file) {
  ...
  reader.onload = function(e) {
    try {
      ...
      const applied = applyImportedStateToMap(state);

      if (applied) {
        showNotification("✅ Posiciones importadas y aplicadas. Cobertura y exportación actualizadas.", "success");
      } else {
        showNotification("✅ Estado importado. Recarga la página si aún no ves los cambios.", "success");
      }
...
function debounceAnalyzeOrphans() {
  ...
  analyzeOrphansTimer = setTimeout(() => {
    analyzeOrphans();
    // Fuente única de verdad: KPI + barra + export deben quedar espejo del mapa actual
    if (globalData) {
      try {
        const stats = computeStatistics(globalData.nucleos, globalData.satellites, globalData.satCandidates, globalData.selected, []);
        updateStatistics(stats);
      } catch (e) {
        console.warn("⚠️ Error actualizando estadísticas tras cambios:", e);
      }
    }
  }, 300);
}
      ...
      <div class="export-header">
        ...
        <button class="close-btn" onclick="this.closest('.export-modal').remove()">×</button>
	      ...
	      <div class="export-content">
	        <div class="export-summary debug-only">
...
function performSpatialJoin() {
  ...
  const satellitesAllExport = (satellites || []).map((sat, si) => {
    if (!isEligibleSatellite(sat)) return null;
    const sDist = String(sat?.distCode || getRowDistrictCode(sat) || '').trim();

    // Nearest ANY buffer (solo para referencia/etiqueta)
    let nearestAny = { dist: Infinity, center: null };
    for (const c of centerInfos) {
      const d = haversineMeters(c.lat, c.lng, sat.lat, sat.lng);
      if (d < nearestAny.dist) nearestAny = { dist: d, center: c };
    }

    // Cobertura real: solo Núcleo fiscal seleccionado + MISMO distrito
    let nearestAllowed = { dist: Infinity, center: null };
    let nearestCrossDistrict = { dist: Infinity, center: null };
    for (const c of centerInfos) {
      if (c.kind !== 'nucleo') continue;
      const d = haversineMeters(c.lat, c.lng, sat.lat, sat.lng);
      const nDist = String(c?.distCode || '').trim();
      if (sDist && nDist && sDist !== nDist) {
        if (d < nearestCrossDistrict.dist) nearestCrossDistrict = { dist: d, center: c };
        continue;
      }
      if (d < nearestAllowed.dist) nearestAllowed = { dist: d, center: c };
    }

    let covered = Number.isFinite(nearestAllowed.dist) && nearestAllowed.dist <= BUFFER_RADIUS_M;

    // Conflicto: existe núcleo dentro de 7.5km pero en otro distrito (y el satélite NO queda cubierto por su distrito)
    const districtConflict = (!covered) && Number.isFinite(nearestCrossDistrict.dist) && nearestCrossDistrict.dist <= BUFFER_RADIUS_M;
    ...
    return {
      ...
      distrito: sat.dist || '',
      distrito_geo: sat.distGeo || '',
      distrito_final: sDist,
      conflicto_distrital: districtConflict ? 'SI' : 'NO',
      conflicto_nucleo_amie: districtConflict ? (nearestCrossDistrict.center?.amie || '') : '',
      conflicto_nucleo_nombre: districtConflict ? (nearestCrossDistrict.center?.label || '') : '',
      conflicto_nucleo_distrito: districtConflict ? (nearestCrossDistrict.center?.distCode || '') : '',
      conflicto_dist_km: districtConflict ? (nearestCrossDistrict.dist / 1000).toFixed(2) : '',
      zona: sat.zona ?? null,
      ...
      lng: sat.lng,
      covered,
      nearestBuffer: nearestAny.center ? {
        kind: nearestAny.center.kind,
        kindName: nearestAny.center.kindName,
        label: nearestAny.center.label,
        amie: nearestAny.center.amie || '',
        distanceMeters: Math.round(nearestAny.dist),
        distanceKm: (nearestAny.dist / 1000).toFixed(2)
      } : null,
      nearestAllowedNucleo: nearestAllowed.center ? {
        amie: nearestAllowed.center.amie || '',
        label: nearestAllowed.center.label,
        distCode: nearestAllowed.center.distCode || '',
        distanceMeters: Math.round(nearestAllowed.dist),
        distanceKm: (nearestAllowed.dist / 1000).toFixed(2)
      } : null,
...
function buildGIEEAssignmentsForExport() {
  ...
  function closestSelectedNucleoNoCut(lat, lng, distCode) {
    const want = String(distCode || '').trim();
    let best = null;
    let bestDist = Infinity;
    for (const c of activeCenters) {
      if (c?.kind !== 'nucleo' || !Number.isFinite(c?.ni)) continue;
      if (want && String(c?.distCode || '').trim() && String(c.distCode).trim() !== want) continue;
  ...
  satellites.forEach((sat, si) => {
    ...
    if (orphanBid) {
      ...
      const hit = findClosestInGridWithinDistrict(activeGrid, sat.lat, sat.lng, BUFFER_RADIUS_M, sat?.distCode || getRowDistrictCode(sat), 1);
    ...
    if (ni == null) {
      ...
      if (tooFar && Number.isFinite(tooFar.ni)) {
        ...
        const near = closestSelectedNucleoNoCut(sat.lat, sat.lng, sat?.distCode || getRowDistrictCode(sat));
        if (near) ni = near.ni;
      }
    }
  ...
  try {
    ...
    const nodes = selectedIdx.map((ni) => {
      ...
      return {
        ...
        distCode: String(n?.distCode || getRowDistrictCode(n) || '').trim(),
        lat: (pos?.lat ?? n.lat),
        lng: (pos?.lng ?? n.lng)
      };
    }).filter(Boolean);
    ...
    for (let i = 0; i < nodes.length; i++) {
      for (let j = i + 1; j < nodes.length; j++) {
        // ✅ No mezclar distritos: solo unir buffers solapados dentro del mismo distrito
        const di = String(nodes[i]?.distCode || '').trim();
        const dj = String(nodes[j]?.distCode || '').trim();
    ...
    for (const item of groupArr) {
      ...
      const primaryDist = String(primary?.distCode || '').trim();

      const satSet = new Set();
      for (let si = 0; si < satellites.length; si++) {
        const s = satellites[si];
        if (!s) continue;
        const sDist = String(s?.distCode || getRowDistrictCode(s) || '').trim();
        if (primaryDist && sDist && primaryDist !== sDist) continue; // ✅ límite distrital
        // satellites ya es fiscal 51–120
        for (const node of g) {
          const nDist = String(node?.distCode || '').trim();
          if (primaryDist && nDist && primaryDist !== nDist) continue;
    ...
    // =========================
    // Conflictos distritales (para reporte)
    // =========================
    try {
      const nucCenters = nodes.map(n => ({ kind: 'nucleo', ni: n.ni, lat: n.lat, lng: n.lng, amie: n.amie, distCode: String(n.distCode || '').trim() }));
      const gridAll = buildPointGrid(nucCenters);
      const conflictRows = [];

      for (let si = 0; si < satellites.length; si++) {
        const s = satellites[si];
        if (!s) continue;
        const sAmie = String(s.amie || '').trim();
        const sDist = String(s?.distCode || getRowDistrictCode(s) || '').trim();

        // 1) ¿Tiene cobertura válida en su distrito?
        const ok = findClosestInGridWithinDistrict(gridAll, s.lat, s.lng, BUFFER_RADIUS_M, sDist, 1);
        if (ok) continue;

        // 2) ¿Está dentro de un núcleo pero de OTRO distrito? => conflicto
        const anyHit = findClosestInGridWithin(gridAll, s.lat, s.lng, BUFFER_RADIUS_M, 1);
        if (!anyHit) continue;
        const n = anyHit.point;
        const nDist = String(n?.distCode || '').trim();
        if (sDist && nDist && sDist === nDist) continue;

        conflictRows.push({
          SATELITE_AMIE: toDash(sAmie),
          SATELITE_DISTRITO: sDist,
          NUCLEO_AMIE_EN_OTRO_DISTRITO: toDash(n?.amie || ''),
          NUCLEO_DISTRITO: nDist,
          DISTANCIA_KM: (anyHit.distance / 1000).toFixed(2),
          OBSERVACION: 'Dentro de 7.5 km, pero cruzando límite distrital (no se asigna)'
        });
      }

      // Se deja disponible para exportación (3ra hoja opcional)
      window._districtConflictsRows = conflictRows;
    } catch (e) {
      console.warn('[DISTRICT] No se pudo construir reporte de conflictos:', e);
      window._districtConflictsRows = [];
    }
...
function exportToExcel() {
  try {
    ...
    // 3) Hoja Conflictos_Distrito (opcional): satélites que caen dentro de un buffer de núcleo,
    //    pero en otro distrito (se quedan SIN cobertura por la regla de límite distrital)
    const conflictRows = Array.isArray(window._districtConflictsRows) ? window._districtConflictsRows : [];
    if (conflictRows.length) {
      const wsC = XLSX.utils.json_to_sheet(conflictRows);
      XLSX.utils.book_append_sheet(wb, wsC, 'Conflictos_Distrito');
    }
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  setupControls();
  setupEditControls();
  // Cargar distritos (si existe el GeoJSON) y luego el CSV.
  // Si falla la capa de distritos, el sistema sigue funcionando con la lógica previa.
  loadDistrictsLayer()
    .catch(e => console.warn('[WARN] No se pudo cargar Distritos_simplified.geojson:', e))
    .finally(() => loadCSV());
});

async function loadDistrictsLayer(){
  try {
    const res = await fetch('Distritos_simplified.geojson', { cache: 'no-store' });
    if (!res.ok) throw new Error('HTTP ' + res.status);
    const gj = await res.json();
    districtsGeoJSON = gj;

    const idx = buildDistrictSpatialIndex(gj);
    districtsSpatialIndex = idx.grid;
    districtsFeatures = idx.feats;

    // Capa de dibujo
    if (districtsLayer) {
      try { layers.districts.removeLayer(districtsLayer); } catch (_) {}
    }
    districtsLayer = L.geoJSON(gj, {
      style: function () {
        return {
          color: '#000000',     // negro
          weight: 3,            // grosor distrital
          opacity: 0.9,
          fillOpacity: 0,
          dashArray: '4,6'
        };
      ...
      onEachFeature: function (feature, layer) {
        const code = normalizeDistrictCode(
          feature && feature.properties && feature.properties.DA_DIST
            ? feature.properties.DA_DIST
            : ''
        );
        layer.bindTooltip(code || 'Distrito', {
          sticky: true,
          direction: 'top'
        });
      }
...
// Por defecto: NO encender distritos automáticamente (solo disponible en el panel)
    console.log('✅ Distritos cargados:', districtsFeatures.length);
  } catch (e) {
    // Mantener estado nulo y continuar.
    districtsGeoJSON = null;
    districtsSpatialIndex = null;
    districtsFeatures = [];
    throw e;
  }
}
...
const buffer = { id: `custom_${customBufferCounter}`, circle, lat, lng, originalPos: { lat, lng }, currentPos: { lat, lng }, isCustom: true, isDragging: false, name: `Buffer Personalizado #${customBufferCounter}` };
  ...
  // 🗑️ Doble clic para eliminar (solo en modo edición o eliminación)
  circle.on('dblclick', (e) => {
    try { L.DomEvent.stopPropagation(e); L.DomEvent.preventDefault(e); } catch(_){ }
    if (!(editMode || deleteMode)) return;
    if (confirm(`¿Eliminar "${buffer.name}"?`)) deleteCustomBuffer(buffer.id);
  });
...
function makeBufferDraggable(circle, data, isCustom, ni = null) {
  ...
  const startDrag = (startEvent) => {
    ...
    isDragging = true;
    data.isDragging = true;
    // 🗑️ Mostrar zona de eliminación durante el arrastre
    showBufferTrash(true);
    setBufferTrashActive(false);
    let overTrash = false;
    ...
    const onDocMove = (ev) => {
      ...
      try {
        overTrash = isClientPointOverTrash(ev.clientX, ev.clientY);
        setBufferTrashActive(overTrash);
      } catch(_){ }
    ...
    const onDocUp = () => {
      ...
      // Ocultar basurero al terminar
      showBufferTrash(false);

      // Si se soltó sobre el basurero: eliminar (solo buffers personalizados)
      if (overTrash) {
        if (isCustom && data?.id) {
          deleteCustomBuffer(data.id);
          return;
        }
        // Núcleos: no se pueden eliminar
        showNotification("⚠️ Solo se pueden eliminar buffers personalizados", "error");
      }
    ...
    const onTouchMove = (ev) => {
      ...
      if (!t) return;
      try {
        overTrash = isClientPointOverTrash(t.clientX, t.clientY);
        setBufferTrashActive(overTrash);
      } catch(_){ }
...
function findClosestInGridWithinDistrict(grid, lat, lng, maxDistM, distCode, ring = 1) {
  // Igual que findClosestInGridWithin, pero filtrando por distCode cuando esté disponible.
  const want = String(distCode || '').trim();
  if (!want) return findClosestInGridWithin(grid, lat, lng, maxDistM, ring);
  const cellLat = Math.floor(lat / GRID_CELL_DEG);
  const cellLng = Math.floor(lng / GRID_CELL_DEG);
  let minDist = Infinity;
  let best = null;
  ...
  for (let dLat = -ring; dLat <= ring; dLat++) {
    for (let dLng = -ring; dLng <= ring; dLng++) {
      ...
      if (!arr) continue;
      for (const p of arr) {
        const pDist = String(p?.distCode || '').trim();
        if (pDist && pDist !== want) continue;
        const dist = haversineMeters(p.lat, p.lng, lat, lng);
        if (dist < minDist) { minDist = dist; best = p; }
      }
    }
...
function getActiveNucleoCentersOnly() {
  ...
  selected?.forEach?.(ni => {
    ...
    if (!isFiscalInstitution(nuc)) return;
    const pos = getCurrentNucleoLatLng(ni, nuc);
    centers.push({ kind: 'nucleo', ni, lat: pos.lat, lng: pos.lng, distCode: String(nuc?.distCode || getRowDistrictCode(nuc) || '').trim() });
  });
...
function processData(data) {
  ...
  // 0) Enriquecer distrito por geometría (si la capa está disponible)
  //    - distGeo: código de distrito desde el polígono
  //    - distCode: código normalizado final usado por la regla de no mezclar distritos
  try {
    for (const d of data) {
      const lat = Number(d?.lat), lng = Number(d?.lng);
      if (!Number.isFinite(lat) || !Number.isFinite(lng)) continue;
      const geoCode = getDistrictCodeForPoint(lat, lng);
      if (geoCode) d.distGeo = geoCode;
      d.distCode = getRowDistrictCode(d);
    }
  } catch (e) {
    console.warn('[WARN] No se pudo enriquecer distritos por geometría:', e);
  }
...
function findCandidates(nucleos, satellites, spatialIndex) {
  ...
  nucleos.forEach((n, ni) => {
    const nDist = String(n?.distCode || getRowDistrictCode(n) || '').trim();
    ...
    for (let dLat = -2; dLat <= 2; dLat++) for (let dLng = -2; dLng <= 2; dLng++) {
      (spatialIndex.get(`${cellLat + dLat},${cellLng + dLng}`) || []).forEach(si => {
        ...
        if (dist <= BUFFER_RADIUS_M) {
          // ✅ Límite distrital: no mezclar satélites de diferentes distritos.
          const s = satellites[si];
          const sDist = String(s?.distCode || getRowDistrictCode(s) || '').trim();
          if (nDist && sDist && nDist !== sDist) return;
...
function drawSatellites(satellites, satCandidates, selected) {
  ...
  satellites.forEach((s, si) => {
    ...
    const hit = findClosestInGridWithinDistrict(activeGrid, s.lat, s.lng, BUFFER_RADIUS_M, s?.distCode || getRowDistrictCode(s), 1);
    if (hit) {
      normalCovered = true;
...
function computeStatistics(nucleos, satellites, satCandidates, selected, nucleoStats) {
  // ✅ Estadísticas coherentes con el modelo y con lo dibujado en el mapa:
  // Universo: Satélites FISCAL (51–120) únicamente
  ...
  satellites.forEach((s, si) => {
    if (!isEligibleSatellite(s)) return;
    eligibleTotal++;
  ...
  // Satélites cubiertos por Buffers B# (solo-satélite) — mismo universo (FISCAL 51–120)
  const bCoveredAll = getBufferBCoveredSet(satellites, nucleos);
  const bCoveredSet = new Set();
  for (const si of bCoveredAll) {
    const s = satellites[si];
    if (s && isEligibleSatellite(s)) bCoveredSet.add(si);
  }

  const totalCovered = new Set([...normalCoveredSet, ...bCoveredSet]).size;
  const uncovered = Math.max(0, eligibleTotal - totalCovered);

  return {
    totalNucleos: nucleos.length,
    totalSatellites: eligibleTotal,
    coveragePercent: eligibleTotal > 0 ? ((totalCovered / eligibleTotal) * 100).toFixed(2) : "0.00",
    totalStudents,
