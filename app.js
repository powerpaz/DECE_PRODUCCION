...
const ORPHAN_MAX_DISTANCE_M = BUFFER_RADIUS_M; // Conectar huérfanos al núcleo más cercano (sin límite de distancia)
...
function sameDistrict(a, b) {
  const da = normDistrict(a);
  const db = normDistrict(b);
  return !!da && !!db && da === db;
}

function sostenimientoTag(row) {
  const s = normStr(row?.sostenimiento).toUpperCase();
  if (!s) return 'NO_REGISTRA';
  if (s.includes('FISCOMISIONAL')) return 'FISCOMISIONAL';
...
function isFiscalInstitution(row) {
  ...
  // Aceptar "FISCAL" y excluir "FISCOMISIONAL"
  if (!sost.includes('FISCAL')) return false;
  if (sost.includes('FISCOMISIONAL')) return false;
  return true;
}

function styleForSatellite(s, attentionStatus) {
  // attentionStatus: 'buffer' | 'distrital' | 'sin'
  const tag = sostenimientoTag(s);
  const isFiscal = isFiscalInstitution(s);

  // Color base por sostenimiento (relleno)
  let fillColor = '#f85149'; // fiscal default
  if (!isFiscal) {
    if (tag === 'FISCOMISIONAL') fillColor = '#a371f7';
    else if (tag === 'PARTICULAR') fillColor = '#8b949e';
    else if (tag === 'MUNICIPAL') fillColor = '#f59e0b';
    else fillColor = '#fb7185';
  }

  // Borde por estado de atención
  let color = '#16a34a';
  let weight = 2;
  let dashArray = null;
  if (attentionStatus === 'distrital') {
    color = '#d29922';
    weight = 3;
  } else if (attentionStatus === 'sin') {
    color = '#7f1d1d';
    weight = 3;
  }

  const radius = attentionStatus === 'sin' ? 8 : 6;
...
function performSpatialJoin() {
  ...
  // Asegurar análisis actualizado para exportación
  if (!orphanAnalysis) {
    try { analyzeOrphans(); } catch (e) { /* noop */ }
  }

  const { nucleos, satellites } = globalData;

  // Para “Spatial Join” (buffers) se usan todas las IE visibles (ya filtradas a Fiscales en processData)
  const allInstitutions = [
    ...nucleos.map(n => ({ ...n, type: 'nucleo' })),
    ...satellites.map(s => ({ ...s, type: 'satellite' }))
  ];

  const buffers = [];
  const totalAMIEsCovered = new Set();
  let totalStudentsInBuffers = 0;

  // Buffers de núcleos (editables)
  editableBuffers.forEach((data, ni) => {
    ...
    totalStudentsInBuffers += result.totalStudents;

    buffers.push({
      bufferId: `buffer_nucleo_${ni}`,
      bufferName: data.nucleo?.name || `Núcleo ${ni}`,
      originalName: data.nucleo?.name || '',
      isCustom: false,
  ...
  // Buffers personalizados: si NO tienen núcleos dentro, se numeran A1, A2, ...
  let anonIdx = 1;
  customBuffers.forEach(buffer => {
    ...
    const reportName = (result.nucleosCount === 0) ? `A${anonIdx++}` : (buffer.name || buffer.id || `Buffer_${anonIdx++}`);

    buffers.push({
      bufferId: buffer.id,
      bufferName: reportName,
      originalName: buffer.name || '',
      isCustom: true,
      centerLat: bufferPos.lat,
      centerLng: bufferPos.lng,
      radiusMeters: BUFFER_RADIUS_M,
  ...
  // ============================
  // Export de satélites (coherente con Dashboard DECE)
  // - Cobertura normal: dentro de buffer de núcleo seleccionado (<=7.5 km y mismo distrito)
  // - Atención distrital: existe núcleo <=7.5 km y mismo distrito (sin marcar “forzada”)
  // - Sin atención: no existe núcleo <=7.5 km (mismo distrito)
  // ============================
  const activeCoverageCenters = getActiveCoverageCenters();
  const activeGrid = buildPointGrid(activeCoverageCenters);

  // Para referencia: “buffer más cercano” considera núcleos seleccionados y buffers personalizados
  const activeCentersAll = getActiveBufferCenters();
  const centerInfos = activeCentersAll.map(c => {
    if (c.kind === 'nucleo') {
      const nuc = nucleos?.[c.ni];
      return { ...c, label: nuc?.name || `Núcleo ${c.ni}`, amie: nuc?.amie || '', kindName: 'Núcleo', distrito: nuc?.dist || '' };
    }
    if (c.kind === 'custom') {
      const b = customBuffers?.find(x => x.id === c.id);
      return { ...c, label: b?.name || c.id || 'Buffer personalizado', amie: '', kindName: 'Personalizado', distrito: '' };
    }
    return { ...c, label: 'Buffer', amie: '', kindName: String(c.kind || 'Buffer'), distrito: '' };
  });

  const satellitesAllExport = (satellites || []).map((sat, si) => {
    // Nearest buffer (solo como referencia)
    ...
    // Cobertura normal (solo núcleos seleccionados + mismo distrito)
    const hit = findClosestInGridWithin(activeGrid, sat.lat, sat.lng, BUFFER_RADIUS_M, 1);
    let covered = false;
    let bufferNi = null;
    let bufferDistance = null;
    if (hit && hit.point?.kind === 'nucleo' && sameDistrict(sat.dist, hit.point.distrito)) {
      covered = true;
      bufferNi = hit.point.ni;
      bufferDistance = hit.distance;
    }

    // Atención distrital (antes “forzada”)
    const distrital = orphanAnalysis?.forcedAssignments?.get?.(si) || null;

    const unattended = !covered && !distrital;

    const status = covered ? 'COBERTURA' : (distrital ? 'ATENCION_DISTRITAL' : 'SIN_ATENCION');
    const statusLabel = covered ? 'Atendida - Cobertura (≤ 7.5 km)'
                      : (distrital ? 'Atendida - Atención distrital (≤ 7.5 km)' : 'Sin atención (sin núcleo ≤ 7.5 km)');

    const nucBuffer = (covered && Number.isFinite(bufferNi)) ? (nucleos?.[bufferNi] || null) : null;
    const nucDistrital = (distrital && Number.isFinite(distrital.ni)) ? (nucleos?.[distrital.ni] || null) : null;

    const extended = (
      (Number.isFinite(bufferDistance) && bufferDistance > ORPHAN_WARNING_DISTANCE_M) ||
      (distrital && Number.isFinite(distrital.distance) && distrital.distance > ORPHAN_WARNING_DISTANCE_M)
    );
    ...
    return {
      ...
      status,
      statusLabel,
      attended: status !== 'SIN_ATENCION',
      unattended,

      // Cobertura por núcleo (buffer seleccionado)
      coveredByNucleoBuffer: covered,
      nucleoBuffer: nucBuffer ? {
        ni: bufferNi,
        name: nucBuffer.name || `Núcleo ${bufferNi}`,
        amie: nucBuffer.amie || '',
        distrito: nucBuffer.dist || '',
        distanceMeters: Math.round(bufferDistance || 0),
        distanceKm: ((bufferDistance || 0) / 1000).toFixed(2)
      } : null,

      // Atención distrital (núcleo sugerido dentro de 7.5 km y mismo distrito)
      distritalAttention: distrital ? {
        ni: distrital.ni,
        nucleoName: nucDistrital ? (nucDistrital.name || `Núcleo ${distrital.ni}`) : `Núcleo ${distrital.ni}`,
        nucleoAmie: nucDistrital?.amie || '',
        distrito: nucDistrital?.dist || '',
        distanceMeters: Math.round(distrital.distance || 0),
        distanceKm: ((distrital.distance || 0) / 1000).toFixed(2)
      } : null,

      // Referencia: buffer más cercano (núcleo o personalizado)
      ...
      extendedConnection: extended
    };
  });

  const satellitesAttendedExport = satellitesAllExport.filter(s => s.attended);
  const satellitesUnattendedExport = satellitesAllExport.filter(s => s.unattended);
  const satellitesExtendedExport = satellitesAllExport.filter(s => s.extendedConnection);

  // Núcleos huérfanos (sin satélite atendido por cobertura)
  ...
  // Resumen coherente con Dashboard DECE (atendida = cobertura + atención distrital)
  const st = orphanAnalysis?.stats || {};
  const totalSat = satellitesAllExport.length;
  const attended = Number.isFinite(st.attended) ? st.attended : satellitesAttendedExport.length;
  const unattended = Number.isFinite(st.unattended) ? st.unattended : satellitesUnattendedExport.length;
  const normalCovered = Number.isFinite(st.normalCovered) ? st.normalCovered : satellitesAllExport.filter(s => s.status === 'COBERTURA').length;
  const distritalCovered = Number.isFinite(st.distritalCovered) ? st.distritalCovered : satellitesAllExport.filter(s => s.status === 'ATENCION_DISTRITAL').length;
  const extendedConnections = Number.isFinite(st.extendedConnections) ? st.extendedConnections : satellitesExtendedExport.length;
  const coveragePercent = (typeof st.attendedPercent === 'string' || typeof st.attendedPercent === 'number')
    ? st.attendedPercent
    : (totalSat > 0 ? ((attended / totalSat) * 100).toFixed(2) : "0.00");

  const totalStudentsAttended = satellitesAttendedExport.reduce((sum, s) => sum + (s.students || 0), 0);
  ...
  return {
    ...
    summary: {
      ...
      attendedSatellites: attended,
      unattendedSatellites: unattended,
      normalCoverage: normalCovered,
      distritalAttention: distritalCovered,
      extendedConnections,
      totalStudentsAttended,
      totalStudentsInBuffers,
      coveragePercent: String(coveragePercent)
    },
    buffers,
    satellitesSummary: {
      total: totalSat,
    ...
    satellitesAttended: satellitesAttendedExport,
    satellitesUnattended: satellitesUnattendedExport,
...
function exportToExcel() {
  ...
  [''],
  ['MÉTRICAS (Dashboard DECE)'],
  ['Satélites totales (51-120, fiscales):', data.summary.totalSatellites],
  ['Atendidas (Cobertura + Atención distrital):', data.summary.attendedSatellites],
  ['Sin atención:', data.summary.unattendedSatellites],
  ['Cobertura normal (≤7.5 km):', data.summary.normalCoverage],
  ['Atención distrital (≤7.5 km):', data.summary.distritalAttention],
  ['Conexiones extendidas (>7 km):', data.summary.extendedConnections],
  ['Cobertura (%):', data.summary.coveragePercent + '%'],
  ['Estudiantes atendidos:', data.summary.totalStudentsAttended],
  [''],
  ['MÉTRICAS (Spatial Join de buffers)'],
  ...
  ['Total AMIEs en buffers:', data.summary.totalAMIEs],
  ['Estudiantes en buffers:', data.summary.totalStudentsInBuffers],
  ['Núcleos (fiscales):', data.summary.totalNucleos],
  ['Núcleos huérfanos:', (data.orphanNucleos?.length ?? 0)]
];
XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet(summaryData), 'Resumen');
  
  const buffersHeaders = ['ID Buffer','Nombre (Reporte)','Nombre original','Tipo','Lat Centro','Lng Centro','Radio (m)','Fue Movido','Total AMIEs','Núcleos','Satélites','Estudiantes'];
  const buffersData = data.buffers.map(b => [b.bufferId,b.bufferName,(b.originalName||''),b.isCustom?'Personalizado':'Original',b.centerLat,b.centerLng,b.radiusMeters,b.wasMoved?'Sí':'No',b.totalAMIEs,b.nucleosCount,b.satellitesCount,b.totalStudents]);
...
if (Array.isArray(data.satellitesAll) && data.satellitesAll.length) {
  const satHeaders = [
    'AMIE Satélite','Nombre Satélite','Estudiantes','Distrito Satélite','Zona','Provincia','Cantón','Lat','Lng',
    'Estado','¿Cobertura normal?','Núcleo (buffer)','AMIE Núcleo (buffer)','Distrito Núcleo (buffer)','Distancia a Núcleo (km)',
    '¿Atención distrital?','Núcleo (distrital)','AMIE Núcleo (distrital)','Distrito Núcleo (distrital)','Distancia distrital (km)',
    '¿Sin atención?','Buffer más cercano','Tipo buffer cercano','Distancia buffer cercano (km)','¿Extendida >7km?'
  ];
  ...
  const satRows = data.satellitesAll.map(s => [
    ...
    s.lat, s.lng,
    s.statusLabel || '',
    s.coveredByNucleoBuffer ? 'Sí' : 'No',
    s.nucleoBuffer?.name || '',
    s.nucleoBuffer?.amie || '',
    s.nucleoBuffer?.distrito || '',
    s.nucleoBuffer?.distanceKm ?? '',
    s.distritalAttention ? 'Sí' : 'No',
    s.distritalAttention?.nucleoName || '',
    s.distritalAttention?.nucleoAmie || '',
    s.distritalAttention?.distrito || '',
    s.distritalAttention?.distanceKm ?? '',
    s.unattended ? 'Sí' : 'No',
    s.nearestBuffer?.label || '',
    s.nearestBuffer?.kindName || '',
    s.nearestBuffer?.distanceKm ?? '',
    s.extendedConnection ? 'Sí' : 'No'
  ]);

  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([satHeaders, ...satRows]), 'Satélites');
}

  // --- Satélites SIN atención ---
if (Array.isArray(data.satellitesUnattended) && data.satellitesUnattended.length) {
  const uncHeaders = ['AMIE','Nombre','Estudiantes','Distrito','Lat','Lng','Buffer más cercano','Distancia buffer cercano (km)'];
  const uncRows = data.satellitesUnattended.map(s => [
    s.amie, s.name, s.students, s.distrito, s.lat, s.lng,
    s.nearestBuffer?.label || '',
    s.nearestBuffer?.distanceKm ?? ''
  ]);
  XLSX.utils.book_append_sheet(wb, XLSX.utils.aoa_to_sheet([uncHeaders, ...uncRows]), 'Satélites_SIN_Atención');
}

  // --- Conexiones extendidas (>7km) ---
if (Array.isArray(data.satellitesExtended) && data.satellitesExtended.length) {
  const extHeaders = ['AMIE','Nombre','Estudiantes','Distrito','Estado','Núcleo (si aplica)','AMIE Núcleo','Distancia (km)'];
  const extRows = data.satellitesExtended.map(s => [
    s.amie, s.name, s.students, s.distrito,
    s.statusLabel || '',
    (s.nucleoBuffer?.name || s.distritalAttention?.nucleoName || ''),
    (s.nucleoBuffer?.amie || s.distritalAttention?.nucleoAmie || ''),
    (s.nucleoBuffer?.distanceKm ?? s.distritalAttention?.distanceKm ?? '')
  ]);
...
if (Array.isArray(data.satellitesAll) && data.satellitesAll.length) {
  const satHeaders = [
    'AMIE','Nombre','Estudiantes','Distrito','Lat','Lng','Estado',
    'CoberturaNormal','NucleoBuffer','AmieNucleoBuffer','DistNucleoBufferKm',
    'AtencionDistrital','NucleoDistrital','AmieNucleoDistrital','DistDistritalKm',
    'SinAtencion','BufferMasCercano','TipoBuffer','DistBufferKm','Extendida>7km'
  ];
  ...
  const satRows = data.satellitesAll.map(s => ([
    ...
    s.lat, s.lng,
    `"${String(s.statusLabel || '').replace(/"/g,'""')}"`,
    s.coveredByNucleoBuffer ? 'SI' : 'NO',
    `"${String(s.nucleoBuffer?.name || '').replace(/"/g,'""')}"`,
    `"${String(s.nucleoBuffer?.amie || '').replace(/"/g,'""')}"`,
    s.nucleoBuffer?.distanceKm ?? '',
    s.distritalAttention ? 'SI' : 'NO',
    `"${String(s.distritalAttention?.nucleoName || '').replace(/"/g,'""')}"`,
    `"${String(s.distritalAttention?.nucleoAmie || '').replace(/"/g,'""')}"`,
    s.distritalAttention?.distanceKm ?? '',
    s.unattended ? 'SI' : 'NO',
    `"${String(s.nearestBuffer?.label || '').replace(/"/g,'""')}"`,
    `"${String(s.nearestBuffer?.kindName || '').replace(/"/g,'""')}"`,
    s.nearestBuffer?.distanceKm ?? '',
    s.extendedConnection ? 'SI' : 'NO'
  ].join(',')));
...
function getActiveBufferCenters() {
  ...
  selected?.forEach?.(ni => {
    ...
    centers.push({ kind: 'nucleo', ni, lat: pos.lat, lng: pos.lng, distrito: nuc.dist, amie: nuc.amie, name: nuc.name });
  });
  ...
  customBuffers.forEach(b => {
    ...
    centers.push({ kind: 'custom', id: b.id, lat: pos.lat, lng: pos.lng, name: b.name || `Buffer ${b.id}` });
  });

  return centers;
}

// Centros que DEFINEN cobertura: solo buffers de núcleos seleccionados (respetando distrito)
...
function getCoverageInfoForSatellite(sat, activeBufferGrid) {
  ...
  if (hit) {
    // Regla: no cruzar distritos
    if (hit.point?.kind === 'nucleo' && !sameDistrict(sat.dist, hit.point.distrito)) {
      return { covered: false, dist: null };
    }
    return { covered: true, dist: hit.distance };
  }
  return { covered: false, dist: null };
}
...
function findClosestFromPool(sat, pool) {
  const satDist = normDistrict(sat.dist);
  let best = null;
  let minDist = Infinity;
  for (const n of pool) {
    // Regla: no cruzar distritos
    if (satDist && n?.distrito && !sameDistrict(satDist, n.distrito)) continue;
    const dist = haversineMeters(n.lat, n.lng, sat.lat, sat.lng);
    if (dist < minDist) { minDist = dist; best = n; }
  }
  if (!best) return null;
  // Regla: solo asignar si existe núcleo a <= 7.5 km
  if (minDist > BUFFER_RADIUS_M) return null;
...
function analyzeOrphans() {
  ...
  // Limpiar
  orphanAnalysis.forcedAssignments.clear(); // ahora representa "Atención distrital" (<=7.5 km)
  orphanAnalysis.orphanSatellites.clear();  // ahora representa "SIN atención" (sin núcleo <=7.5 km)
  orphanAnalysis.unservedSatellites.clear();
  orphanAnalysis.orphanNucleos.clear();

  let normalCovered = 0;            // dentro de buffer de núcleo seleccionado (<=7.5 km) y mismo distrito
  let distritalCovered = 0;         // fuera de buffer pero con núcleo <=7.5 km (mismo distrito)
  let extendedConnections = 0;      // atendidas con distancia > 7 km

  // Índice de cobertura: SOLO buffers de núcleos seleccionados
  const activeCoverageCenters = getActiveCoverageCenters();
  const activeGrid = buildPointGrid(activeCoverageCenters);

  // Lookup de núcleos para atención distrital (respeta provincia/isla + distrito)
  const lookup = buildNucleoLookup(nucleos, selected);

  // Para identificar núcleos sin satélites (solo seleccionados)
  const hasAnySatellite = new Array(nucleos.length).fill(false);

  satellites.forEach((sat, si) => {
    // 1) Cobertura por buffer (núcleo seleccionado) respetando distrito
    const cov = getCoverageInfoForSatellite(sat, activeGrid);
    if (cov.covered) {
      normalCovered++;
      const hit = findClosestInGridWithin(activeGrid, sat.lat, sat.lng, BUFFER_RADIUS_M, 1);
      if (hit?.point?.kind === 'nucleo' && Number.isFinite(hit.point.ni)) hasAnySatellite[hit.point.ni] = true;
      if (cov.dist != null && cov.dist > ORPHAN_WARNING_DISTANCE_M) extendedConnections++;
      return;
    }

    // 2) Atención distrital: SOLO si existe núcleo <= 7.5 km (mismo distrito)
    ...
    if (closest) {
      ...
      if (closest.distance > ORPHAN_WARNING_DISTANCE_M) extendedConnections++;
      return;
    }

    // 3) SIN atención: no existe núcleo <= 7.5 km (mismo distrito)
  ...
  const attended = normalCovered + distritalCovered;
  const unattended = Math.max(0, satellites.length - attended);
  ...
  orphanAnalysis.stats = {
    ...
    attendedPercent: satellites.length ? ((attended / satellites.length) * 100).toFixed(2) : "0.00"
  };

  console.log("Total satélites:", satellites.length);
  console.log("Cobertura normal:", normalCovered, `(${orphanAnalysis.stats.normalPercent}%)`);
  console.log("Atención distrital:", distritalCovered);
  console.log("Atendidas:", attended, `(${orphanAnalysis.stats.attendedPercent}%)`);
  console.log("Sin atención:", unattended);
  console.log("Conexiones extendidas (>7km):", extendedConnections);
  console.log("Núcleos huérfanos:", orphanAnalysis.orphanNucleos.size);
...
function updateOrphanPanel() {
  ...
  panel.innerHTML = `
    ...
    <div class="orphan-stat">
      <div class="stat-label">Atención distrital</div>
      <div class="stat-value">${s.distritalCovered}</div>
      <div class="stat-sub">satélites (≤7.5 km)</div>
    </div>
    <div class="orphan-stat warn">
      <div class="stat-label">Conexiones extendidas (&gt;7km)</div>
      <div class="stat-value">${s.extendedConnections}</div>
      <div class="stat-sub">satélites atendidos</div>
    </div>
    <div class="orphan-stat" style="background: rgba(220, 38, 38, 0.08); border-left: 4px solid #dc2626;">
      <div class="stat-label" style="color:#dc2626;">Sin atención</div>
      <div class="stat-value" style="color:#dc2626;">${s.unattended}</div>
      <div class="stat-sub">sin núcleo ≤7.5 km</div>
    </div>
    <div class="orphan-stat highlight">
      <div class="stat-label">COBERTURA SATÉLITES</div>
      <div class="stat-value-big">${s.attendedPercent}%</div>
      <div class="stat-sub">${s.attended} de ${s.total}</div>
    </div>
    <div class="orphan-stat">
...
function processData(data) {
  ...
  // ✅ Se trabaja solo con IE FISCALES (núcleos y satélites).
...
// ✅ Filtro 1: trabajar SOLO con IE fiscales (núcleos y satélites)
nucleos = nucleos.filter(isFiscalInstitution);
satellitesAll = satellitesAll.filter(isFiscalInstitution);
console.log(`[FILTER] IE fiscales: núcleos=${nucleos.length}, satélites=${satellitesAll.length}`);
...
function drawNucleos(nucleos, selected) {
  // Icono SVG (tipo “escudo”) para diferenciar núcleos visualmente
  const makeIcon = (isSelected) => {
    const size = isSelected ? 28 : 22;
    const fill = isSelected ? '#1e40af' : '#60a5fa';
    const stroke = '#ffffff';
    const svg = `
      <svg width="${size}" height="${size}" viewBox="0 0 64 64" xmlns="http://www.w3.org/2000/svg">
        <path d="M32 3 L55 13 V31 C55 44 45 55 32 61 C19 55 9 44 9 31 V13 Z"
              fill="${fill}" stroke="${stroke}" stroke-width="3" />
        <text x="32" y="38" text-anchor="middle" font-size="20" font-family="Inter, Arial" fill="#0b1220" font-weight="800">N</text>
      </svg>`;
    return L.divIcon({
      className: '',
  ...
  nucleos.forEach((n, ni) => {
    ...
    const marker = L.marker([n.lat, n.lng], { icon: makeIcon(isSelected), keyboard: false });
...
function drawSatellites(satellites, satCandidates, selected) {
  // satCandidates/selected se mantienen por compatibilidad.
  // Cobertura = SOLO por buffers de NÚCLEOS seleccionados, respetando distrito.
  const activeCoverageCenters = getActiveCoverageCenters();
  const activeGrid = buildPointGrid(activeCoverageCenters);

  let bufferCount = 0;
  let distritalCount = 0;
  let sinCount = 0;
  // Alias visual para satélites SIN núcleo (sin atención): B1, B2, B3...
  let noNucleoAliasCounter = 0;
  ...
  satellites.forEach((s, si) => {
    ...
    // 1) Cobertura por buffer de núcleo seleccionado
    const cov = getCoverageInfoForSatellite(s, activeGrid);
    if (cov.covered) {
      normalCovered = true;
      normalDist = cov.dist;
      const hit = findClosestInGridWithin(activeGrid, s.lat, s.lng, BUFFER_RADIUS_M, 1);
      if (hit?.point?.kind === 'nucleo' && Number.isFinite(hit.point.ni)) bufferNi = hit.point.ni;
    }

    // 2) Atención distrital (<=7.5 km) si NO está cubierto por buffer
    const distritalAssign = (!normalCovered && orphanAnalysis?.forcedAssignments?.get(si))
      ? orphanAnalysis.forcedAssignments.get(si)
      : null;

    const attentionStatus = normalCovered ? 'buffer' : (distritalAssign ? 'distrital' : 'sin');

    // Asignar alias B# a satélites sin atención (sin núcleo cercano)
    // Se usa para identificarlos rápidamente en el mapa y en reportes/popup.
    if (attentionStatus === 'sin') {
      noNucleoAliasCounter++;
      s.alias = `B${noNucleoAliasCounter}`;
    } else {
      // Limpiar alias previo si existía (evita arrastre entre recálculos)
      delete s.alias;
    }

    const marker = L.circleMarker([s.lat, s.lng], styleForSatellite(s, attentionStatus));
    marker.bindPopup(createSatellitePopup(s, { normalCovered, normalDist, bufferNi, distrital: distritalAssign }));

    if (attentionStatus === 'buffer') {
      bufferCount++;
      marker.addTo(layers.satellites);
    } else if (attentionStatus === 'distrital') {
      distritalCount++;
      marker.addTo(layers.satellites);
    } else {
      sinCount++;
      ...
      // Etiqueta permanente B# para satélites sin núcleo
      if (s.alias) {
        marker.bindTooltip(s.alias, {
          permanent: true,
          direction: 'top',
          className: 'sat-alias-label',
          offset: [0, -10],
          opacity: 0.95
        });
      }
  ...
  console.log('%c═══════════════════════════════════', 'color: #9333ea; font-weight: bold;');
  console.log('%c🟢 Cobertura por buffer: ' + bufferCount, 'color: #10b981; font-size: 14px; font-weight: bold;');
  console.log('%c🟡 Atención distrital: ' + distritalCount, 'color: #d29922; font-size: 14px; font-weight: bold;');
  console.log('%c🔴 Sin atención: ' + sinCount, 'color: #dc2626; font-size: 14px; font-weight: bold;');
  console.log('%c📊 Total satélites: ' + satellites.length, 'color: #60a5fa; font-size: 14px;');
  console.log('%c═══════════════════════════════════', 'color: #9333ea; font-weight: bold;');
  
  if (sinCount > 0) {
    console.log('%c✅ HAY ' + sinCount + ' SATÉLITES SIN ATENCIÓN - Usa el toggle para verlos', 'background: #dc2626; color: white; padding: 8px; font-weight: bold; font-size: 12px;');
  } else {
    console.log('%c✅ 100% COBERTURA - Todos los satélites están atendidos', 'background: #10b981; color: white; padding: 8px; font-weight: bold; font-size: 12px;');
  }
}
...
function createNucleoPopup(n, satCount, totalStudents) {
  const sost = String(n?.sostenimiento || '').trim();
  const isFiscal = isFiscalInstitution(n);
  const sostLabel = sost ? sost : (isFiscal ? 'FISCAL' : 'NO REGISTRA');

  return `<div class="popup-title">🏛️ Núcleo DECE</div>` +
    `<div class="popup-content">` +
      `<div class="popup-row"><span class="popup-label">AMIE:</span><span class="popup-value">${escapeHTML(n.amie || '')}</span></div>` +
      `<div class="popup-row"><span class="popup-label">Institución:</span><span class="popup-value">${escapeHTML(n.name)}</span></div>` +
      `<div class="popup-row"><span class="popup-label">Sostenimiento:</span><span class="popup-value">${escapeHTML(sostLabel)}</span></div>` +
      `<div class="popup-row"><span class="popup-label">Distrito:</span><span class="popup-value">${escapeHTML(n.dist)}</span></div>` +
      `<div class="popup-row"><span class="popup-label">Estudiantes:</span><span class="popup-value" style="color:#d29922">${(n.students || 0).toLocaleString()}</span></div>` +
    `</div>`;
}

function createSatellitePopup(s, info) {
  // info: { normalCovered, normalDist, bufferNi, distrital: {ni,distance}|null }
  const normalCovered = !!(info && info.normalCovered);
  const normalDist = (info && typeof info.normalDist === 'number') ? info.normalDist : null;
  const bufferNi = (info && Number.isFinite(info.bufferNi)) ? info.bufferNi : null;
  const distrital = info && info.distrital ? info.distrital : null;

  let statusColor = "#dc2626";
  let statusText = "🔴 SIN atención (sin núcleo ≤ 7.5 km)";
  let badge = "🔴";
  if (normalCovered) {
    statusColor = "#10b981";
    statusText = "✅ ATENDIDA - Con cobertura (≤ 7.5 km)";
    badge = "🟢";
  } else if (distrital) {
    statusColor = "#d29922";
    statusText = "🟡 ATENDIDA - Atención distrital (≤ 7.5 km)";
    badge = "🟡";
  }

  const sost = String(s?.sostenimiento || '').trim();
  const isFiscal = isFiscalInstitution(s);
  const sostLabel = sost ? sost : (isFiscal ? 'FISCAL' : 'NO REGISTRA');

  const aliasLine = s?.alias
    ? `<div class="popup-row"><span class="popup-label">Alias:</span><span class="popup-value" style="color:#f97316;font-weight:700">${escapeHTML(s.alias)}</span></div>`
    : '';
  ...
  let nucleoLine = '';
  if (normalCovered && bufferNi !== null && globalData?.nucleos?.[bufferNi]) {
    const nuc = globalData.nucleos[bufferNi];
    nucleoLine = `<div class="popup-row"><span class="popup-label">Núcleo:</span><span class="popup-value" style="color:#1e40af">${escapeHTML(nuc.name)} (${escapeHTML(nuc.amie || '')})</span></div>`
      + `<div class="popup-row"><span class="popup-label">Distrito Núcleo:</span><span class="popup-value">${escapeHTML(nuc.dist || '')}</span></div>`;
  } else if (distrital && globalData?.nucleos?.[distrital.ni]) {
    const nuc = globalData.nucleos[distrital.ni];
    nucleoLine = `<div class="popup-row"><span class="popup-label">Atención por:</span><span class="popup-value" style="color:#d29922">${escapeHTML(nuc.name)} (${escapeHTML(nuc.amie || '')})</span></div>`
      + `<div class="popup-row"><span class="popup-label">Distrito Núcleo:</span><span class="popup-value">${escapeHTML(nuc.dist || '')}</span></div>`
      + `<div class="popup-row"><span class="popup-label">Distancia:</span><span class="popup-value">${(distrital.distance/1000).toFixed(2)} km</span></div>`;
  }

  return `<div class="popup-title">📍 Satélite ${badge}</div>` +
    `<div class="popup-content">` +
      aliasLine +
      `<div class="popup-row"><span class="popup-label">AMIE:</span><span class="popup-value">${escapeHTML(s.amie || '')}</span></div>` +
...
function computeStatistics(nucleos, satellites, satCandidates, selected, nucleoStats) {
  // ✅ Estadísticas coherentes con el panel DECE:
  // - Atendida = (a) dentro de buffer de núcleo seleccionado (<=7.5 km y mismo distrito)
  //           o (b) atención distrital (existe núcleo <=7.5 km y mismo distrito)
  // - Sin atención = no existe núcleo <=7.5 km (mismo distrito)
  const activeCoverageCenters = getActiveCoverageCenters();
  const activeGrid = buildPointGrid(activeCoverageCenters);
  const lookup = buildNucleoLookup(nucleos, selected);
  ...
  satellites.forEach((s) => {
    ...
    const closest = findClosestNucleoForOrphan(s, lookup, true);
    if (closest) {
      attended++;
      totalStudents += s.students || 0;
    }
  ...
  const uncovered = Math.max(0, total - attended);

  return {
    totalNucleos: nucleos.length,
    totalSatellites: total,
    coveragePercent: total > 0 ? ((attended / total) * 100).toFixed(2) : "0.00",
    totalStudents,
