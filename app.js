...
function drawForcedConnections(targetLinesArray = null) {
  ...
  orphanAnalysis.forcedAssignments.forEach((assign, si) => {
    ...
    const tag = assign.distance <= ORPHAN_WARNING_DISTANCE_M ? "Asignación (≤ 7 km)" : "Asignación (> 7 km)";
...
const findClosestAnyOverall = (lat, lng) => {
  ...
  satellites.forEach((sat, si) => {
    ...
    if (covered) {
      ...
      if (withinNormal) {
        ...
        // Satélite huérfano: asignar al núcleo más cercano (sin límite)
      const closest = (selected?.size ? findClosestSelectedOverall(sat.lat, sat.lng) : null) || findClosestAnyOverall(sat.lat, sat.lng);
      if (closest) {
        orphanAnalysis.forcedAssignments.set(si, { ni: closest.ni, distance: closest.distance });
        forcedCovered++;
        // Si supera el umbral (7 km), lo marcamos como conexión extendida para reporte
        ...
        // Sin núcleos disponibles: queda sin atención
        orphanAnalysis.orphanSatellites.add(si);
      }
      }
    }
  ...
  // Redibujar conexiones para incluir asignaciones forzadas al núcleo más cercano
  ...
  if (coverageElement) {
    ...
    coverageElement.innerHTML = `
      <div style="text-align: center;">
        <div style="font-size: 2em; color: #00ff88;">${stats.totalCoveragePercent}%</div>
        <div style="font-size: 0.8em; color: #8b949e;">
          Base: ${stats.normalCoveragePercent}% 
          <span style="color: #00ff88;">+${improvement}%</span>
        </div>
      </div>
    `;
  }
...
if (!nucleos.length || !satellites.length) {
  ...
  // ========== CONECTAR SATÉLITES ROJOS ==========
  setTimeout(() => {
    console.log("🔗 CONECTANDO SATÉLITES ROJOS...");
    
    // Crear capa de conexiones
    if (!layers.satelliteConnections) {
      layers.satelliteConnections = L.featureGroup().addTo(map);
    } else {
      layers.satelliteConnections.clearLayers();
    }
    
    const MAX_DIST = 7000; // 7km
    let connected = 0;
    ...
    satellites.forEach((sat, si) => {
      ...
      if (satCandidates[si]) {
        satCandidates[si].forEach(c => {
          if (result.selected.has(c.ni) && c.dist <= BUFFER_RADIUS_M) {
            covered = true;
          }
        });
      }
      
      // Si NO está cubierto, buscar núcleo cercano
      if (!covered) {
        let closestNi = null;
        let minDist = MAX_DIST;
        
        result.selected.forEach(ni => {
          const nuc = nucleos[ni];
          if (!nuc) return;
          ...
          const dist = Math.sqrt(
            Math.pow((sat.lat - lat) * 111000, 2) + 
            Math.pow((sat.lng - lng) * 111000 * Math.cos(sat.lat * Math.PI / 180), 2)
          );
        ...
        // Dibujar línea verde
        if (closestNi !== null) {
          const nuc = nucleos[closestNi];
          const bufData = editableBuffers.get(closestNi);
          const lat = bufData?.currentPos?.lat || nuc.lat;
          const lng = bufData?.currentPos?.lng || nuc.lng;
          
          const line = L.polyline(
            [[sat.lat, sat.lng], [lat, lng]],
            {
              color: '#00ff88',
              weight: 2,
              opacity: 0.7,
          ...
          line.bindPopup(`
            <b style="color: #00ff88;">✓ CONECTADO</b><br>
            <b>Satélite:</b> ${sat.name}<br>
            <b>→ Núcleo:</b> ${nuc.name}<br>
            <b>Distancia:</b> ${(minDist/1000).toFixed(2)} km
          `);
          
          layers.satelliteConnections.addLayer(line);
          connected++;
        }
      }
    });
    
    console.log(`✅ ${connected} satélites conectados con líneas verdes`);
  ...
  // Conectar satélites desatendidos
  setTimeout(() => {
    console.log("🔗 Iniciando análisis de conexiones...");
    const stats = connectUnattendedSatellites(nucleos, satellites, satCandidates, result.selected);
    drawAnimatedConnections(nucleos, satellites);
    updateCoverageDisplay(stats);
  }, 1000);
}
...
function drawSatellites(satellites, satCandidates, selected) {
  satellites.forEach((s, si) => {
    ...
    // Si tiene asignación forzada al núcleo más cercano, marcar como cubierto
    const forced = orphanAnalysis?.forcedAssignments?.get(si);
    if (!isCovered && forced) { isCovered = true; bestDist = forced.distance; }
