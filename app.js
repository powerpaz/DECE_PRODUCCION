...
function recomputeOrphanBufferCoverage() {
  ...
  orphanBuffersState.list.forEach(ob => {
    ...
    satellites.forEach((sat, si) => {
      ...
      if (!isFiscalInstitution(sat) || isExcludedGroup1to50(sat)) return;
...
function regenerateAnimations() {
  ...
  satellites.forEach((sat, si) => {
    // ✅ Regla de negocio: SOLO SATÉLITES FISCALES (y no excluidos 1–50) generan conexiones
    if (!isFiscalInstitution(sat) || isExcludedGroup1to50(sat)) return;
    let assigned = null;
...
function analyzeOrphans() {
  ...
  let normalCovered = 0;
  let forcedCovered = 0;
  let eligibleTotal = 0; // ✅ Solo satélites fiscales (no excluidos)
  ...
  // Analizar satélites
  satellites.forEach((sat, si) => {
    // ✅ Regla de negocio: SOLO SATÉLITES FISCALES (y no excluidos 1–50) entran al modelo
    if (!isFiscalInstitution(sat) || isExcludedGroup1to50(sat)) return;
    eligibleTotal++;
  ...
  orphanAnalysis.stats = {
    ...
    totalPercent: eligibleTotal ? (((normalCovered + forcedCovered) / eligibleTotal) * 100).toFixed(2) : "0.00"
  };
