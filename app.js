...
function refreshBSNBufferLabels() {
  // Añade etiquetas "BSN" a buffers que contienen Núcleo + al menos 1 Satélite Fiscal (51–120) dentro de 7.5 km.
  // Nota: NO limpia layers.bufferLabels (para no borrar alias B# ya dibujados por refreshOrphanBufferLabels()).
  try {
    if (!layers?.bufferLabels) return;
    if (!globalData || !globalData.satellites) return;

    const sats = globalData.satellites.filter(s => isFiscalInstitution(s) && isSatellite51to120(s));
    ...
    editableBuffers.forEach((data) => {
      ...
      for (let i = 0; i < sats.length; i++) {
        const sat = sats[i];
        const d = haversineMeters(sat.lat, sat.lng, pos.lat, pos.lng);
        if (d <= BUFFER_RADIUS_M) { count++; if (count >= 1) break; }
      }
      if (count < 1) return;

      const html = `
        <div style="
          width:36px;height:22px;border-radius:10px;
      ...
      L.marker([pos.lat, pos.lng], { icon, interactive: false }).addTo(layers.bufferLabels);
    });
  } catch (e) {
    console.warn("refreshBSNBufferLabels error:", e);
  }
}
...
function updateOrphanBuffers() {
  ...
  if (!globalData) {
    ...
    try { refreshBSNBufferLabels(); } catch(_) {}
return;
  }
...
function buildGIEEAssignmentsForExport() {
  ...
  const res = {
    ...
    bsnNucleoAmies: new Set() // núcleos con al menos 1 satélite fiscal cubierto
  };
  ...
  satellites.forEach((sat, si) => {
    ...
    if (ni != null && nucleos?.[ni]?.amie) {
      const nAmie = String(nucleos[ni].amie || '').trim();
      if (nAmie) {
        res.amieNucleoByAmie.set(satAmie, nAmie);
        if (covered) res.bsnNucleoAmies.add(nAmie);
      }
    }
  });
...
function exportToExcel() {
  try {
    ...
    const bsnNucleoAmies = gieeAssign?.bsnNucleoAmies || new Set();
    // Exponer para otras partes (etiquetas BSN)
    try { window._bsnNucleoAmies = bsnNucleoAmies; } catch(_) {}
    ...
    function classifyForMap(r){
      ...
      if (!sat) {
        if (fiscal) {
          const a0 = String(r?.amie || '').trim();
          return (a0 && bsnNucleoAmies?.has?.(a0)) ? 'NUCLEO_FISCAL_BSN' : 'NUCLEO_FISCAL';
        }
        return 'NUCLEO_OTRO';
      }
      ...
      if (dece === '1') {
        const nA = gieeAssign?.amieNucleoByAmie?.get?.(a);
        return (nA && bsnNucleoAmies?.has?.(String(nA).trim())) ? 'SAT_CUBIERTO_BSN' : 'SAT_CUBIERTO';
      }
      return 'SAT_HUERFANO';
    }
    ...
    function isVisibleCategory(cat){
      switch(cat){
        ...
        case 'NUCLEO_FISCAL_BSN': return !!vis.nucleosFiscal;
        case 'NUCLEO_OTRO': return !!vis.nucleosOther;
        case 'SAT_CUBIERTO': return !!vis.satCovered;
        case 'SAT_CUBIERTO_BSN': return !!vis.satCovered;
    ...
    function buildMapExplainRow(r){
      ...
      const estado = (cat === 'SAT_CUBIERTO' || cat === 'SAT_CUBIERTO_BSN') ? 'CUBIERTO (≤7.5 km)' :
