...
function buildCoberturaZonasFromMapState() {
  const rows = [];
  const allBuffers = [...editableBuffers, ...customBuffers];
  ...
  allBuffers.forEach((buffer, idx) => {
    ...
    orphanBufferCoverageState?.satAssignments?.forEach((info, satId) => {
...
function performSpatialJoin() {
  ...
  const satAssignments = orphanBufferCoverageState?.satAssignments;

  // Fuente preferida: globalData.satellites (si existe) para conservar metadata
  const baseSatList = (globalData?.satellites && Array.isArray(globalData.satellites))
